...
function create_document_link($www,$title,$path,$filetype,$size,$visibility)
{
	...
	//QUITAMOS EL ENLACE DE DESCARGA DE LA CARPETA	//$forcedownload_link=($filetype=='folder')?api_get_self().'?'.api_get_cidreq().'&action=downloadfolder&amp;path='.$url_path.$req_gid:api_get_self().'?'.api_get_cidreq().'&amp;action=download&amp;id='.$url_path.$req_gid;
$forcedownload_link=($filetype=='folder')?'':api_get_self().'?'.api_get_cidreq().'&amp;action=download&amp;id='.$url_path.$req_gid;

	//folder download or file download?
	//QUITAMOS EL ICONO folder_zip DE DESCARGA DE LA CARPETA
	//$forcedownload_icon=($filetype=='folder')?'folder_zip.gif':'filesave.gif';
	$forcedownload_icon=($filetype=='folder')?'':'filesave.gif';
	...
	//CREAMOS UNA VARIABLE PARA DEVOLVER EL ENLACE CON DESCARGA DE ARCHIVO Y SIN ICONO DE DESCARGA CUANDO ES UNA CARPETA
	$cadena_descarga = '<a href="'.$url.'" title="'.$tooltip_title.'" target="'.$target.'"'.$visibility_class.' style="float:left">'.$title.'</a>';
	if ($filetype=='file'){
		$cadena_descarga.= $force_download_html;
	}
	return $cadena_descarga;
}
