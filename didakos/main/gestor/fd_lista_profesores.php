...
Página actual: fd_lista_profesores.php
Descripción: Página que muestra un listado de los profesores. 
...
function get_number_of_users()
{
	...
	$sql = "SELECT COUNT(u.user_id) AS total_number_of_items FROM $user_table u WHERE u.user_id>1 and u.status=1";
...
function get_user_data($from, $number_of_items, $column, $direction)
{
	...
	$sql = "SELECT
		 ...
		 u.password			AS col9,
             ...
             FROM
                 $user_table u where u.status=1 and user_id>1";
	if (isset ($_GET['keyword']))
	{
		$keyword = Database::escape_string($_GET['keyword']);
...
function modify_filter($user_id,$url_params,$row)
{
	...
	//Egarcia 6/9/11
	//Habilitamos estadísticas de profesores.
...
function active_filter($active, $url_params, $row)
{
	...
	{
		$result = '<a href="fd_lista_profesores.php?action='.$action.'&amp;user_id='.$row['0'].'&amp;'.$url_params.'&amp;sec_token='.$_SESSION['sec_token'].'"><img src="../img/'.$image.'.gif" border="0" style="vertical-align: middle;" alt="'.get_lang(ucfirst($action)).'" title="'.get_lang(ucfirst($action)).'"/></a>';
	}
	return $result;
}
...
if (isset ($_GET['search']) && $_GET['search'] == 'advanced')
{
	...
	$interbreadcrumb[] = array ("url" => 'fd_lista_profesores.php', "name" => get_lang('UserList'));
...
{
	$interbreadcrumb[] = array ("url" => 'index.php', "name" => get_lang('PlatformAdmin'));
	$tool_name = get_lang("ListadoDeProfesores");
	Display :: display_header($tool_name, "");
	//api_display_tool_title($tool_name);
	...
	$form->addElement('static','search_advanced_link',null,'<a href="fd_lista_profesores.php?search=advanced">'.get_lang('AdvancedSearch').'</a>');
	$form->display();
	if (isset ($_GET['keyword']))
	{
