...
Página incial: fd_desmatricular_1.php (FD versión 0.3)
Página actual: fd_seg_diplomas_1.php
Descripción: Página que muestra un listado de los alumnos del curso seleccionado en el 
que se muestran las descargas realizadas y la última fecha en la que se ha realizado.
...
$interbreadcrumb[] = array ("url" => 'index.php', "name" => get_lang('PlatformAdmin'));
$tool_name = "Seguimiento de diplomas";
Display :: display_header($tool_name);
...
$sql = 'SELECT u.firstname,u.lastname,u.official_code,u.username,u.password,cu.status as course_status, ds.f_entrega, count(ds.id_diploma) as descargas
FROM '.$table_user.' u inner join '.$table_course_user." cu on (u.user_id=cu.user_id)
LEFT OUTER JOIN diplomas_seguimiento ds ON (ds.user_id = u.user_id)
...
if (mysql_num_rows($res) > 0)
{
	...
	$table->set_header(6,'Descargas', true);
	$table->set_header(7,'Fecha');
	$table->display();
}
else
{
