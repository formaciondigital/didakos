...
Autor: Jesús Zafra
Página incial: user_add.php (1.8.5)
Página actual: fd_profesor_add.php
Descripción: Página que crea un nuevo profesor en la plataforma
...
include_once ($libpath.'usermanager.lib.fd.php');
...
// Official code
$form->addElement('text', 'official_code', get_lang('OfficialCode'),array('size' => '10','value'=>$_GET['dni']));
...
//fdupdate - se elimina el campo username ya que vamos a generarlo nosotros
...
//fdupdate - se elimina la opcion rellenar el password,se genera automaticamente
...
//fdupdate - el valor del campo status 
...
/*$form->addElement('text','status',get_lang('Status'));*/
...
$form->addElement('hidden','status',get_lang('Status'),get_lang('Learner'),'id="status_select"');*/
// Send email
/*$group = array();
...
if( $form->validate())
{
	...
	if($check)
	{
		...
		$user_id = UserManagerFD::create_teletutor($firstname,$lastname,$status,$email,$official_code,api_get_setting('platformLanguage'),$phone,$picture_uri,$auth_source,$expiration_date,$active, $hr_dept_id);
		if($user_id == 0)//el dni ya existe devolvemos un error
		{
		  die("ese dni ya existe en la base de datos");
		}
		...
		if(isset($user['submit_plus']))
		{
			...
			header('Location: fd_profesor_add.php?message='.urlencode(get_lang('UserAdded').'&sec_token='.$tok));
			exit ();
		}
		else
		{
				//fdupdate - modificado para que redirija al listado de alumnos modificado
			$tok = Security::get_token();
			header('Location: fd_lista_profesores.php?action=show_message&message='.urlencode(get_lang('UserAdded').'&sec_token='.$tok));
			exit ();
		}
	}
}else{
