...
Autor: Jesús Zafra
Página incial: user_edit.php (1.8.5)
Página actual: fd_profesor_edit.php
Descripción: Página de modificación de profesores
...
include(api_get_path(LIBRARY_PATH).'usermanager.lib.fd.php');
...
{
	header('Location: fd_lista_profesores.php');
	exit;
}
...
// Official code
$form->addElement('text', 'official_code', get_lang('OfficialCode'),array('size' => '10'));
...
$drh_list = UserManager :: get_fd_lista_profesores(array('status'=>DRH),array('lastname','firstname'));
...
$form->addGroup($group, 'mail', get_lang('SendMailToNewUser'), '&nbsp;',false);*7
...
if( $form->validate())
{
	...
	UserManagerFD::update_user($user_id,$firstname,$lastname,$username,$password,$auth_source,$email,$status,$official_code,$phone,$picture_uri,$expiration_date, $active, null, $hr_dept_id);
	...
	header('Location: fd_lista_profesores.php?tipo=profesor&action=show_message&message='.urlencode(get_lang('UserUpdated')));
	exit();
}
