...
Página incial: user_list.php (1.8.5)
Página actual: fd_lista_alumnos.php
Descripción: Página que muestra un listado de los alumnos quitando la opción de eliminar 
...
function get_number_of_users()
{
	...
	$sql = "SELECT COUNT(u.user_id) AS total_number_of_items FROM $user_table u WHERE u.user_id>1 and u.status=5";
	if (isset ($_GET['keyword']))
	{
		$keyword = Database::escape_string($_GET['keyword']);
		$sql .= " AND (u.firstname LIKE '%".$keyword."%' OR u.lastname LIKE '%".$keyword."%'  OR u.email LIKE '%".$keyword."%'  OR u.official_code LIKE '%".$keyword."%')";
	}
	elseif (isset ($_GET['keyword_firstname']))
	{
		...
		$sql .= $query_admin_table .
				" AND (u.firstname LIKE '%".$keyword_firstname."%' " .
...
function get_user_data($from, $number_of_items, $column, $direction)
{
	...
	$sql = "SELECT
		 ...
		 u.status,
                 ...
                 u.user_id			AS col8
             FROM
                 $user_table u where u.status=5 and user_id>1";
	if (isset ($_GET['keyword']))
	{
		$keyword = Database::escape_string($_GET['keyword']);
		$sql .= " and (u.firstname LIKE '%".$keyword."%' OR u.lastname LIKE '%".$keyword."%'  OR u.username LIKE '%".$keyword."%'  OR u.official_code LIKE '%".$keyword."%')";
	}
	elseif (isset ($_GET['keyword_firstname']))
	{
		$admin_table = Database :: get_main_table(TABLE_MAIN_ADMIN);
		...
		$keyword_active = isset($_GET['keyword_active']);
		$keyword_inactive = isset($_GET['keyword_inactive']);
		$sql .= $query_admin_table." and (u.firstname LIKE '%".$keyword_firstname."%' " .
				"AND u.lastname LIKE '%".$keyword_lastname."%' " .
				"AND u.username LIKE '%".$keyword_username."%'  " .
				"AND u.email LIKE '%".$keyword_email."%'   " .
				"AND u.official_code LIKE '%".$keyword_officialcode."%')" .
...
function modify_filter($user_id,$url_params,$row)
{
	...
	$result .= '<a href="fd_lista_alumnos.php?action=login_as&amp;user_id='.$user_id.'&amp;sec_token='.$_SESSION['sec_token'].'"><img src="../img/login_as.gif" border="0" style="vertical-align: middle;" alt="'.get_lang('LoginAs').'" title="'.get_lang('LoginAs').'"/></a>&nbsp;';

	//egarcia 30/12/09 Eliminamos el filtro para que el gstor tenga acceso.
	...
	$result .= '<a href="fd_alumno_edit.php?user_id='.$user_id.'"><img src="../img/edit.gif" border="0" style="vertical-align: middle;" title="'.get_lang('Edit').'" alt="'.get_lang('Edit').'"/></a>&nbsp;';
...
function active_filter($active, $url_params, $row)
{
	...
	{
		$result = '<a href="fd_lista_alumnos.php?action='.$action.'&amp;user_id='.$row['0'].'&amp;'.$url_params.'&amp;sec_token='.$_SESSION['sec_token'].'"><img src="../img/'.$image.'.gif" border="0" style="vertical-align: middle;" alt="'.get_lang(ucfirst($action)).'" title="'.get_lang(ucfirst($action)).'"/></a>';
	}
	return $result;
}
...
if (isset ($_GET['search']) && $_GET['search'] == 'advanced')
{
	...
	$interbreadcrumb[] = array ("url" => 'fd_lista_alumnos.php', "name" => get_lang('UserList'));
...
else
{
	...
	$form->addElement('static','search_advanced_link',null,'<a href="fd_lista_alumnos.php?search=advanced">'.get_lang('AdvancedSearch').'</a>');
	$form->display();
	if (isset ($_GET['keyword']))
	{
	...
	$table->set_header(5, get_lang('Password'));
	$table->set_header(6, get_lang('Email'));
	$table->set_header(7, get_lang('Phone'));
	$table->set_header(8, get_lang('Status'));
	$table->set_header(9, get_lang('Active'));	
	$table->set_header(10, get_lang('Modify'));
