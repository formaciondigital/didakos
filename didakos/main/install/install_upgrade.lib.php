...
==============================================================================
*/
define("DOKEOS_MAIN_DATABASE_FILE", "db_creation.sql");
...
define("COURSES_HTACCESS_FILENAME", "course_htaccess.dist");
define("DOKEOS_CONFIG_FILENAME", "configuration.dist.php");
define("DOKEOS_MAIL_CONFIG_FILENAME", "mail.conf.dist.php");
 ...
 */
/*function write_main_htaccess_file()
{
	
	
        
        $content = file_get_contents("main_htaccess.dist");
	$content = str_replace('{HTACCESS_INDEX}', 'index.html', $content);
       
        $fil = fopen("../../.htaccess",'w');
        if ($fil)
	{
		fwrite($fil, $content);
                return fclose($fil);
...
function write_dokeos_config_file($path)
{
	...
	if (!$fp)
	{
		echo '<b><font color="red">Your script doesn\'t have write access to the config directory</font></b><br />
						...
						You probably do not have write access on root directory,
...
/**
 * Write the mail config file
 * @param string $path Path to the config file
 */
function write_dokeos_mail_config_file($path)
{
	        global $SmtpFromEmail;
                global $SmtpFromName;
                global $SmtpHost;
                global $SmtpUser;
                global $SmtpPass;
                global $ServicioTecnico;
	...
	$file_path = dirname(__FILE__).'/'.DOKEOS_MAIL_CONFIG_FILENAME;
	$content = file_get_contents($file_path);

	$config['{SMTP_FROM_EMAIL}'] = $SmtpFromEmail;
	$config['{SMTP_FROM_NAME}'] = $SmtpFromName;
	$config['{SMTP_HOST}'] = $SmtpHost;
	$config['{SMTP_USER}'] = $SmtpUser;
	$config['{SMTP_PASS}'] = $SmtpPass;
        //si no recibimos usuario es que no necesita autentificacion
        $config['{SMTP_AUTH}'] = ($SmtpHost != "" && $SmtpUser == "" && $SmtpPass) ? 0 : 1;
        
	$config['{SERVICIO_TECNICO}'] = $ServicioTecnico;
