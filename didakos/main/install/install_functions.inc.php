...
function display_language_selection_box()
{
	...
	$language_to_display = array('Brasileño' => 'brazilian_fd','Inglés' => 'english_fd','Francés' => 'french_fd','Español' => 'spanish_fd','Catalán' => 'catalan_fd');

	//display
	echo "\t\t<select name=\"language_list\">\n";

	$default_language = 'spanish_fd';
...
function display_requirements($installType, $badUpdatePath, $updatePath='', $update_from_version_8=array(), $update_from_version_6=array())
{
	echo '<h2>'.display_step_sequence().get_lang('Requirements')."</h2>\n";
        ...
        /* cambio - eliminamos el enlace a la guia de instalacion*/
	...
	echo '<table class="requirements">
			<tr>
				<td class="requirements-item">'.get_lang('PHPVersion').'>= 5.2</td>
	...
	echo '<table class="requirements">
			<tr>
				<td class="requirements-item">didakos/main/inc/conf/</td>
				<td class="requirements-value">'.check_writable('inc/conf/').'</td>
			</tr>
			<tr>
				<td class="requirements-item">didakos/main/garbage/</td>
				<td class="requirements-value">'.check_writable('garbage/').'</td>
			</tr>
			<tr>
				<td class="requirements-item">didakos/main/upload/</td>
				<td class="requirements-value">'.check_writable('upload/').'</td>
			</tr>
			<tr>
				<td class="requirements-item">didakos/main/default_course_document/images/</td>
				<td class="requirements-value">'.check_writable('default_course_document/images/').'</td>
			</tr>
			<tr>
				<td class="requirements-item">didakos/archive/</td>
				<td class="requirements-value">'.check_writable('../archive/').'</td>
			</tr>
			<tr>
				<td class="requirements-item">didakos/courses/</td>
	...
	else
	{
		...
		<input type="submit" name="step2_install" value="<?php echo get_lang("Next"); ?> >" <?php if($error) echo 'disabled="disabled"'; ?> />
...
function display_database_settings_form($installType, $dbHostForm, $dbUsernameForm, $dbPassForm, $dbPrefixForm, $enableTrackingForm, $singleDbForm, $dbNameForm, $dbStatsForm, $dbScormForm, $dbUserForm)
{
	if($installType == 'update')
	{
		...
		if(empty($dbUserForm))
		{
			...
			{
				$dbUserForm=$dbPrefixForm.'bd_plataforma_user';
			}
		}
	...
	//fields for the four standard Dokeos databases
        //
        //eliminamos los parametros opcionales
	...
	<tr>
		...
		<?php else:
				...
				<div style="float:left;">
                                    <strong><?php echo get_lang('ExisteBBDD'); ?></strong><br />
	...
	<tr>
	  ...
	  <td align="right">
           <?php if($dbConnect==1) : ?>
              <input type="submit" name="step4" value="<?php echo get_lang('Next'); ?> &gt;" />
           <?php else: ?>
              <input type="submit" name="step4" value="<?php echo get_lang('Next'); ?> &gt;" disabled/>
           <?php endif; ?>
          </td>
	</tr>
	</table>
	...
	
{
	...
	display_configuration_parameter($installType, get_lang("InstituteURL"), "institutionUrlForm", $institutionUrlForm);
        ...
        echo "<tr><td><br/><b>".  get_lang('langPlatformAdmin')."</b></td></tr>";
	...
	if($installType != 'update')
		display_configuration_parameter($installType, get_lang("AdminPass"), "passForm", $passForm, false);
         ...
         echo "<tr><td><br/><b>".  get_lang('gestor')."</b></td></tr>";
	//gestor  email
	display_configuration_parameter($installType, get_lang("AdminEmail"), "gestoremailForm", $gestoremailForm);

	//gestor last name
	display_configuration_parameter($installType, get_lang("AdminLastName"), "gestorLastName", $gestorLastName);

	//gestor first name
	display_configuration_parameter($installType, get_lang("AdminFirstName"), "gestorFirstName", $gestorFirstName);

	//gestor login
	display_configuration_parameter($installType, get_lang("AdminLogin"), "gestorloginForm", $gestorloginForm, ($installType == 'update' ? true : false));

	//gestor password
	if($installType != 'update')
		display_configuration_parameter($installType, get_lang("AdminPass"), "gestorpassForm", $gestorpassForm, false);

         echo "<tr><td>&nbsp;</td></tr>";
	//Parameter 9: campus name
         
            echo "<tr><td><b>".  get_lang('ServTecnico')."</b></td></tr>";
        display_configuration_parameter($installType, get_lang("Email"), "ServicioTecnico", $ServicioTecnico);
         
            echo "<tr><td><br/><b>".  get_lang('Email')."</b></td></tr>";
	
	display_configuration_parameter($installType, get_lang("langSmtpFromEmail"), "SmtpFromEmail", $SmtpFromEmail);

	
	display_configuration_parameter($installType, get_lang("langSmtpFromName"), "SmtpFromName", $SmtpFromName);

	
	display_configuration_parameter($installType, get_lang("langSmtpHost"), "SmtpHost", $SmtpHost);

        
	display_configuration_parameter($installType, get_lang("langSmtpUser"), "SmtpUser", $SmtpUser);

	
	display_configuration_parameter($installType, get_lang("langSmtpPass"), "SmtpPass", $SmtpPass);
	...
	<tr>
	  <td><input type="hidden" name="allowSelfReg" value="<?php echo $allowSelfReg; ?>" /></td>
	</tr>
	<tr>
	  <td><input type="hidden" name="allowSelfRegProf" value="<?php echo $allowSelfRegProf; ?>" /></td>
	</tr>
	<tr>
			...
			if($multipleDbCheck !== false)
			{
				...
				else
				{
					$dbConnect = 0;
                               ...
                               //comprobamos tambien que exista ya un esquema con ese prefijo
                                $r_existeDb = @mysql_query("SELECT 1 FROM `information_schema`.`SCHEMATA`  where SCHEMA_NAME = '".$dbPrefixForm."_main' ");
                                $num_rows = mysql_num_rows($r_existeDb);
