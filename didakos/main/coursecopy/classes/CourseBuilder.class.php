...
class CourseBuilder
{
	...
	function ExtrasBuilder()
	{
	    global $_extras;
		$this->extras = new Course();
		$this->extras->code = $_extras['official_code'];
		$this->extras->path = api_get_path(SYS_COURSE_PATH).$_extras['path'].'/';
		$this->extras->backup_path = api_get_path(SYS_COURSE_PATH).$_extras['path'];
	}
	...
	function build_extras()
	{
        ...
        $sql = "select * from $table_glosario order by id";
        $db_result = api_sql_query($sql, __FILE__, __LINE__);
		while ($obj = Database::fetch_object($db_result))
		{
			$glosario = new Glosario($obj->id, $obj->palabra, $obj->descripcion);
			$this->course->add_resource($glosario);
		}
		
		// Multimedia
	    $table_multimedia = Database :: get_course_table(TABLE_MULTIMEDIA);
	    $sql = "select * from $table_multimedia order by id";
        $db_result = api_sql_query($sql, __FILE__, __LINE__);
		while ($obj = Database::fetch_object($db_result))
		{
			$multimedia = new Multimedia($obj->id, $obj->title, $obj->description, $obj->duration, $obj->source_id, $obj->target);
			$this->course->add_resource($multimedia);
		}
		return $this->course;
	}
