...
class CourseArchiver
{
	 ...
	 */
	function write_course($course,$extras)
	{
		...
		$extras_file = $backup_dir.'extras.dat';
        ...
        // Solo escribimos el archivo si hay algún contenido
        if( (is_array($extras->resources[RESOURCE_GLOSARIO]) && (count($extras->resources[RESOURCE_GLOSARIO])>=1)) || ( is_array($extras->resources[RESOURCE_MULTIMEDIA]) && (count($extras->resources[RESOURCE_MULTIMEDIA])>=1)) )
		{
            // Escribimos extras		
		    $fp = @fopen($extras_file, 'w');
		    if($fp == false)
		    {
          			error_log(__FILE__.' line '.__LINE__.': '.(ini_get('track_errors')!=false?$php_errormsg:'error not recorded because track_errors is off in your php.ini'),0);
		    } 
		    $res = @fwrite($fp,   base64_encode(serialize($extras)));
		    if($res == false)
		    {
          			error_log(__FILE__.' line '.__LINE__.': '.(ini_get('track_errors')!=false?$php_errormsg:'error not recorded because track_errors is off in your php.ini'),0);
		    } 
		    ...
		    // Copy al multimedia
            if( is_array($extras->resources[RESOURCE_MULTIMEDIA]))
		    {
			    foreach ($extras->resources[RESOURCE_MULTIMEDIA] as $id => $multimedia)
			    {
				    if ($multimedia->source_id == 5)
				    {
					    $doc_dir=dirname($backup_dir.'multimedia');
        				mkdirr($doc_dir,0755);
					    copyDirTo($extras->path.'multimedia', $doc_dir, false);
					    //salimos del bucle
				       break;
				    }
			    }
	...
	function read_extras($ruta)
	{
        @chdir($ruta);
        $fp = @fopen('extras.dat', "r");
		$contents = @fread($fp, filesize('extras.dat'));
		@fclose($fp);
    	$extras = unserialize(base64_decode($contents));
		$extras->backup_path = $ruta;
		return $extras;
	}
