...
// Extras
require_once ('Glosario.class.php');
...
class CourseRestorer
{
	...
	function restore_extras($destination_course_code = '')
	{
	    if ($destination_course_code == '')
		{
			$course_info = api_get_course_info();
		...
		if (count($this->course)>=1)
		{
		    // Glosario
			$table = Database :: get_course_table(TABLE_GLOSARIO, $this->course->destination_db);			
			$resources = $this->course->resources;
			foreach ($resources[RESOURCE_GLOSARIO] as $id => $glosario)
			{
				$sql = "INSERT INTO ".$table." " .
						"SET palabra = '".Database::escape_string($glosario->palabra)."'," .
							"descripcion = '".Database::escape_string($glosario->descripcion)."'";
				api_sql_query($sql, __FILE__, __LINE__);
				$this->course->resources[RESOURCE_GLOSARIO][$id]->destination_id = Database::get_last_insert_id();
			}
            // Multimedia
			$table = Database :: get_course_table(TABLE_MULTIMEDIA, $this->course->destination_db);
			$resources = $this->course->resources;
			foreach ($resources[RESOURCE_MULTIMEDIA] as $id => $multimedia)
			{
				$sql = "INSERT INTO ".$table." " .
						"SET title= '".Database::escape_string($multimedia->title)."'," .
                        "description= '".Database::escape_string($multimedia->description)."'," .
                        "duration= '".Database::escape_string($multimedia->duration)."'," .
                        "source_id= ".Database::escape_string($multimedia->source_id)."," .
                        "target='".Database::escape_string($multimedia->target)."'," .
                        "date=now(),user_id=2";                       
				api_sql_query($sql, __FILE__, __LINE__);
				$this->course->resources[RESOURCE_MULTIMEDIA][$id]->destination_id = Database::get_last_insert_id();
			}                       
			copyDirTo($this->course->backup_path.'/multimedia', api_get_path(SYS_COURSE_PATH).$this->course->destination_path, false);
		}	
	}
