...
class CourseSelectForm
{
	 ...
	 */
	function display_form($course, $extras, $hidden_fields = null)
	{
		...
		// recursos extras
		$resource_titles[RESOURCE_MULTIMEDIA] = get_lang('Multimedia');
		$resource_titles[RESOURCE_GLOSARIO] = get_lang('Glosario');
...
?>
		...
		echo '<input type="hidden" name="extras" value="'.base64_encode(serialize($extras)).'"/>';              
		...
		// Mostramos ahora los recursos extras.
		foreach ($extras->resources as $type => $resources)
		{
			if (count($resources) > 0)
			{
				echo ' <img id="img_'.$type.'" src="../img/1.gif" onclick="javascript:exp('."'$type'".');" >';
	...
	function get_posted_extras()
	{
		$course = unserialize(base64_decode($_POST['extras']));
