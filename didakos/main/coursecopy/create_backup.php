...
if ((isset ($_POST['action']) && $_POST['action'] == 'course_select_form') || (isset ($_POST['backup_option']) && $_POST['backup_option'] == 'full_backup'))
{
	...
	else
	{
		...
		$course = $cb->build();	
        // Obtenemos los recursos extras
        $ce = new CourseBuilder();
        $extras = $ce->build_extras();
	}
	$zip_file = CourseArchiver :: write_course($course,$extras);
...
elseif (isset ($_POST['backup_option']) && $_POST['backup_option'] == 'select_items')
{
	...
	CourseSelectForm :: display_form($course,$extras);
}
else
{
	$cb = new CourseBuilder();
