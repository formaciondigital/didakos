...
$csv_content = array();
 ...
 if(isset($_GET['details']))
 {
 	...
 	else if(!empty($_GET['origin']) && $_GET['origin'] == 'tracking_course')
 	{
 		...
 		if(empty($cidReq))
 			$interbreadcrumb[] = array ("url" => api_get_path(WEB_COURSE_PATH).$course_infos['directory'], 'name' => $course_infos['title']);
 		$interbreadcrumb[] = array ("url" => "../tracking/courseLog.php?cidReq=".Security::remove_XSS($_GET['course']).'&studentlist=true&id_session='.(empty($_SESSION['id_session'])?'':$_SESSION['id_session']), "name" => get_lang("Tracking"));
 	}
 	else
 	{
 ...
 else
 {
 	...
 	if(isset($_GET['id_coach']) && intval($_GET['id_coach'])!=0){
 		...
 		$interbreadcrumb[] = array ("url" => "student.php?id_coach=".Security::remove_XSS($_GET['id_coach'])."&id_session=".Security::remove_XSS($_GET['id_session']), "name" => get_lang("CoachStudents"));
 		}
 		else{
 			$interbreadcrumb[] = array ("url" => "student.php?id_coach=".Security::remove_XSS($_GET['id_coach']), "name" => get_lang("CoachStudents"));
 		}
...
if(empty($_SESSION['is_allowedCreateCourse']) && !api_is_coach() && $_user['status']!=DRH && $_user['status']!=SESSIONADMIN){
	if ($_SESSION['is_platformAdmin']!=1)
	{	
...
if(!empty($_GET['student']))
{
	...
	echo '<div align="right">
		...
		<a href="'.api_get_self().'?'.strip_tags($_SERVER['QUERY_STRING']).'&export=csv"><img align="absbottom" src="../img/excel.gif">&nbsp;'.get_lang('ExportAsCSV').'</a>
	  </div>';
	...
	//FIN MODIFICACION TIEMPO CURSO
	
	//INICIO MODIFICACION RESULTADOS
	...
	//MODIFICACION TIEMPO CURSO
	$time_spent_on_the_platform = api_time_to_hms(Tracking::get_time_spent_on_the_course($a_infosUser['user_id'], $_GET['course']));
								...
								<tr>
									<td>
										<table>
											...
											<?php
											/*
											//egarcia 30/12/09 Eliminamos esto ya que no es correcto.
											<tr>
												<td class="none" align="right">
							...
							<table width="100%">
                                ...
                                <?php
                                            ...
                                            {   //only show link to connection details if course and student were defined in the URL
                                                echo '<tr>';
                                                echo '<td class="noLink none">';
                                                echo '<img align="absbottom" src="../img/statistics.gif">&nbsp; <strong> <a href="access_details.php?student='.Security::remove_XSS($_GET['student']).'&course='.Security::remove_XSS($_GET['course']).'">'.get_lang('AccessDetails').'</a> </strong>';
                                                echo '</td>';
                                                echo '</tr>';
                                            }
                                ?>
				...
				?>
					<tr class="<?php echo $s_css_class;?>">
						...
						<td align="center">
							...
							<?php*/
							echo '<a href="../auth/my_progress_details_course.php?student='.$a_infosUser['user_id'].'&details=true&course='.$course_infos['code'].'&origin=tracking_course"><img src="'.api_get_path(WEB_IMG_PATH).'2rightarrow.gif" border="0" /></a>';
							}
							?>
						</td>
					</tr>
				...
				if(Database::num_rows($resultExercices)>0)
				{
					while($a_exercices = Database::fetch_array($resultExercices))
					{
						$sqlEssais = "	SELECT COUNT(ex.exe_id) as essais
										...
										AND exe_user_id='".intval($_GET["student"])."'"
									 ;
						...
						$sqlScore = "SELECT exe_id, exe_result,exe_weighting
									 ...
									 WHERE exe_user_id = ".intval($_GET['student'])."
						...
						$sql_last_attempt='SELECT exe_id FROM '.$tbl_stats_exercices.' WHERE exe_exo_id="'.$a_exercices['id'].'" AND exe_user_id="'.intval($_GET['student']).'" AND exe_cours_id="'.$a_infosCours['code'].'" ORDER BY exe_date DESC LIMIT 1';
					...
					<tr>				
						...
						if(isset($_GET['id_coach']) && intval($_GET['id_coach'])!=0){
							echo '<td align="center" width="10">
								<a href="'.api_get_self().'?student='.$a_infosUser['user_id'].'&details=true&course='.$course_infos['code'].'&id_coach='.Security::remove_XSS($_GET['id_coach']).'#infosStudent"><img src="'.api_get_path(WEB_IMG_PATH).'2rightarrow.gif" border="0" /></a>
							</td>';
						}
						else{//egarcia 30/12/09 Hemos eliminado el enlace y redireccionado a nuestro seguimiento.
							echo '<td align="center" width="10">
								...
								<a href="../auth/my_progress_details_course.php?student='.$a_infosUser['user_id'].'&details=true&course='.$course_infos['code'].'&origin=tracking_course"><img src="'.api_get_path(WEB_IMG_PATH).'2rightarrow.gif" border="0" /></a>
							</td>';
						}
					echo '</tr>';
				}
