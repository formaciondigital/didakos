...
if(api_is_allowed_to_create_course() && $view=='teacher')
{
	...
	if($nb_teacher_courses)
	{
		...
		// ***** Consulta modificada por carga excesiva. Solo mostramos ahora el nombre del curso y el número
		// ***** de alumnos en el curso. @egarcia 21/09/11
		...
		//añadimos títulos del informe de FD para el profesor	
		...
		//Comentado array que almacena información de CSV
		/*
		$csv_content[] = array(
		...
		*/						
		...
		foreach($a_courses as $course)
		{
			...
			$sql = "SELECT count(user_id) as total FROM $tbl_course_user as course_rel_user WHERE course_rel_user.status='5' AND course_rel_user.course_code='$course_code'";
			...
			*/
			...
			$nb_students_in_course = $row["total"];
			...
			//	$avg_messages_in_dropbox += Tracking_fd ::count_dmail ($row['user_id'], $course_code);
			...
			*/
			...
			{ // Obtenemos el resultado de los cálculos anteriores divididos entre el número de alumnos del curso
			
				// tiempo medio por alumno
				//$avg_time_spent_in_course = api_time_to_hms($avg_time_spent_in_course / $nb_students_in_course);			
			
				// número medio de mensajes en foros por alumno
				//$avg_messages_in_course = round($avg_messages_in_course / $nb_students_in_course,2);
			
				// número medio de mensajes en buzón por alumno
				//$avg_messages_in_dropbox = round($avg_messages_in_dropbox / $nb_students_in_course,2);
			
				// nota media en exámenes por alumno
			...
			*/	
			...
			$table_row = array();
			$table_row[] = $course['title'] . ' [' . $course_code . ']';
			...
			*/
			$table_row[] = '<a href="../tracking/courseLog_FD.php?cidReq='.$course_code.'&studentlist=true"><img src="'.api_get_path(WEB_IMG_PATH).'2rightarrow.gif" border="0" /></a>';
...
// Comentado informe para el administrador (el administrador verá su página original)
...
*/ //Fin comentado parte del administrador
