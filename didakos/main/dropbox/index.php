...
if (!$_GET['view'] OR $_GET['view']=='received' OR $dropbox_cnf['sent_received_tabs']==false)
{
	...
	if ($view_dropbox_category_received<>0)
	{
		...
		echo '<img src="../img/folder_up.gif" align="absmiddle" /><a href="'.api_get_self().'?'.api_get_cidreq().'&view_received_category=0&amp;view_sent_category='.$_GET['view_sent_category'].'&amp;view='.$_GET['view'].'">'.get_lang('Root')."</a>\n";
	}
	...
	echo "<a href=\"".api_get_self()."?".api_get_cidreq()."&view=".$_GET['view']."&amp;action=addreceivedcategory\"><img src=\"../img/folder_new.gif\" align=\"absmiddle\" /> ".get_lang('AddNewCategory')."</a>\n";
	...
	$column_header[] = array(get_lang('Type'),true,'style="width:40px"');
        $column_header[] = array(get_lang($dropbox_lang["sender"]), TRUE, '');  //$column_header[] = array('Remitente', TRUE, '');
        $column_header[] = array(get_lang($dropbox_lang['title']), TRUE, '');  
        $column_header[] = array(get_lang('Message'), TRUE, '');	
	$column_header[] = array(get_lang('Date'), true);
	...
	foreach ( $dropbox_person -> receivedWork as $dropbox_file)
	{
		...
		if ($view_dropbox_category_received==$dropbox_file->category) // we only display the files that are in the category that we are in.
		{
			...
			if ($dropbox_file->title <> 'Correo')
			{
			$dropbox_file_data[]=build_document_icon_tag('file',$dropbox_file->title).'('.ceil(($dropbox_file->filesize)/1024).''.get_lang('kB').')';
			}else{
			$dropbox_file_data[]=build_document_icon_tag('file',$dropbox_file->title);
			}
			$dropbox_file_data[]=$dropbox_file->author;
			$dropbox_file_data[]='<a href="'.api_get_self().'?'.api_get_cidreq().'&view_received_category='.$_GET['view_received_category'].'&amp;view_sent_category='.$_GET['view_sent_category'].'&amp;view='.$_GET['view'].'&amp;action=viewfeedback&amp;id='.$dropbox_file->id.'">'.$dropbox_file->asunto.'</a>';
			$dropbox_file_data[]=$dropbox_file->description;
			$fecha_formateada = date("d-m-Y H:i:s",strtotime($dropbox_file->last_upload_date));
			
			
			
			
			//aqui se podria meter algo que detecte la fecha y cambie el color o muestre un icono si es reciente		
			$dropbox_file_data[]=date_to_str_ago($dropbox_file->last_upload_date).'<br><span class="dropbox_date">'.$fecha_formateada.'</span>';
			// al guardar si no lleva adjunto se guarda en el campo title 'Correo' para identificar los contenidos con o sin descarga
			if ($dropbox_file->title <> 'Correo')
			{
			$descarga = '<a href="dropbox_download.php?'.api_get_cidreq().'&id='.$dropbox_file->id.'&amp;action=download"><img src="../img/filesave.gif" align="absmiddle" alt="'.get_lang('Save').'" /></a>';
			}
			$action_icons=check_number_feedback($dropbox_file->id, $number_feedback).' '.get_lang('Feedback').'
									<a href="'.api_get_self().'?'.api_get_cidreq().'&view_received_category='.$_GET['view_received_category'].'&amp;view_sent_category='.$_GET['view_sent_category'].'&amp;view='.$_GET['view'].'&amp;action=viewfeedback&amp;id='.$dropbox_file->id.'"><img src="../img/comment_bubble.gif" alt="'.get_lang('Comment').'" align="absmiddle" /></a>' . $descarga . 
			...
			{
				$action_icons.="</td><td>&nbsp;</td></tr>"; // ending the normal row of the sortable table
				//$action_icons.='<tr><td colspan="2"><a href="index.php?"'.api_get_cidreq().'&view_received_category='.$_GET['view_received_category']."&amp;view_sent_category=".$_GET['view_sent_category']."&amp;view=".$_GET['view']."\">".get_lang('CloseFeedback')."</a></td><td colspan=\"6\">".feedback($dropbox_file->feedback2)."</td>\n</tr>\n";
				$action_icons.='<tr><td colspan="8">'.feedback($dropbox_file->feedback2).'</td></tr>';
			...
			$action_icons='';
             ...
             if($dropbox_file->lastfeedback == '-'){
			 $fecha_reenvio = $fecha_formateada;
			 }else{
			 $fecha_reenvio = $dropbox_file->lastfeedback;
			 }
			 
			$dropbox_file_data[]= '<span class="dropbox_date">' . $fecha_reenvio .'</span>';
			//$dropbox_file_data[]= "-";
	...
	if ($view_dropbox_category_received==0)
	{
		foreach ($dropbox_categories as $category) // note: this can probably be shortened since the categories for the received files are already in the $dropbox_received_category array;
		{
			...
			if ($category['received']=='1')
			{
				...
				$dropbox_category_data[]='<a href="'.api_get_self().'?'.api_get_cidreq().'&view_received_category='.$category['cat_id'].'&amp;view_sent_category='.$_GET['view_sent_category'].'&amp;view='.$_GET['view'].'">'.$category['cat_name'].'</a>';
	...
	Display::display_sortable_config_table($column_header, $dropbox_data_recieved, $sorting_options, $paging_options, $additional_get_parameters,$column_show,$column_order, array ('delete_received' => get_lang('Delete'),'download_received'=>get_lang('Download')),'DESC');
}
...
if ($_GET['view']=='sent' OR $dropbox_cnf['sent_received_tabs']==false)
{
	...
	if ($view_dropbox_category_sent<>0)
	{
		...
		echo '<img src="../img/folder_up.gif" align="absmiddle" /><a href="'.api_get_self().'?'.api_get_cidreq().'&view_received_category='.$_GET['view_received_category'].'&amp;view_sent_category=0&amp;view='.$_GET['view'].'">'.get_lang('Root')."</a>\n";
	}
	echo "<a href=\"".api_get_self()."?".api_get_cidreq()."&view=".$_GET['view']."&amp;action=add\"><img src=\"../img/submit_file.gif\" align=\"absmiddle\"/> ".get_lang('UploadNewFile')."</a>&nbsp;\n";
	...
	$column_header[] = array('',false,'');
	$column_header[] = array(get_lang('Type'),true,'style="width:40px"');
	$column_header[] = array(get_lang($dropbox_lang['sendTo']), TRUE, '');
	$column_header[] = array(get_lang($dropbox_lang['title']), TRUE, '');  //$column_header[] = array('asunto', TRUE, '');
	$column_header[] = array(get_lang('Message'), TRUE, '');
	$column_header[] = array(get_lang('Date'), true);
	$column_header[] = array(get_lang('Modify'), FALSE, '');
	$column_header[] = array(get_lang('LastResent'), true);
	...
	if ($view_dropbox_category_sent==0)
	{
		foreach ($dropbox_categories as $category)
		{
			...
			if ($category['sent']=='1')
			{
				...
				$dropbox_category_data[]='<a href="'.api_get_self().'?'.api_get_cidreq().'&view_received_category='.$_GET['view_received_category'].'&amp;view_sent_category='.$category['cat_id'].'&amp;view='.$_GET['view'].'">'.$category['cat_name'].'</a>';
				//$dropbox_category_data[]=''; 
				$dropbox_category_data[]='';
				$dropbox_category_data[]='';
				$dropbox_category_data[]='';
	...
	$additional_get_parameters=array('view'=>$_GET['view'], 'view_received_category'=>$_GET['view_received_category'],'view_sent_category'=>$_GET['view_sent_category']);
    Display::display_sortable_config_table($column_header, $dropbox_data_sent, $sorting_options, $paging_options, $additional_get_parameters,$column_show,$column_order, array ('delete_received' => get_lang('Delete'),'download_received'=>get_lang('Download')),$dir_ordenacion);
}
}
