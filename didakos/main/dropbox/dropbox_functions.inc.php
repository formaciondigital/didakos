...
function display_add_form()
{
	...
	?>
	<form name="form" method="post" action="index.php?view_received_category=<?php echo $_GET['view_received_category']; ?>&view_sent_category=<?php echo $_GET['view_sent_category']; ?>&view=<?php echo $_GET['view']; ?>&<?php echo "&origin=$origin"; ?>" enctype="multipart/form-data" onsubmit="return checkForm(this)">
	<table border="0">
		<tr>
			<td  align="right">
	...
	?>
	  <!-- CAMBIO - se añade un nuevo campo de formulario para almacenar el asunto del buzon -->
	   		<tr>
			<td valign="top"  align="left">
			  <?php echo get_lang("Asunto")?>:
			</td>
			<td>
				<input type="text" name="asunto" id="asunto" size="70">
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<textarea name="description" cols="24" rows="8" style="width: 450px;" onKeyDown="valida_longitud()" onKeyUp="valida_longitud()"></textarea>
			</td>
		</tr>
		<tr>
		  <td align="right"><?php echo get_lang("Caracteres")?>:</td>
   			<td><input type="text" name="caracteres" size="4"> Max. 1000</td>
		</tr> 
		<tr>
			<td valign="top"  align="left">
	...
	if(isset($_SESSION['id_session'])){
		...
		{
			$complete_user_list_for_dropbox =  CourseManagerFD :: get_teacher_list_from_course_code_sin_admin($course_info['code']);
		}
	...
	?>" multiple style="width: 450px;height: 300px;">
	<?php
...
function store_add_dropbox()
{
	...
	*/
	...
	new Dropbox_SentWork( $_user['user_id'], $dropbox_title, $_POST['description'], strip_tags($_POST['authors']), $dropbox_filename, $dropbox_filesize, $new_work_recipients, $_POST['asunto']);
...
/**
* Esta función devuelve la última fecha de los comentarios.
*
...
{
	$fecha_formateada = date("d-m-Y H:i:s",strtotime($feedback['feedback_date'])); 
	$output.=display_user_link($feedback['author_user_id']);
	$output.='&nbsp;&nbsp;['. $fecha_formateada .']<br>';
