...
class Dropbox_Work {
	 ...
	 * @return Dropbox_Work
		*/
	//CAMBIO - añadimos un nuevo argumento para almacenar el asunto
	function Dropbox_Work ($arg1, $arg2=null, $arg3=null, $arg4=null, $arg5=null, $arg6=null , $arg7=null)
	{
		if (func_num_args()>1) 
		{
		    $this->_createNewWork($arg1, $arg2, $arg3, $arg4, $arg5, $arg6 ,$arg7);
		} 
		else 
		{
	...
	//CAMBIO FD - añadimos el parametro ausnto a la creacion del correo	
	function _createNewWork ($uploader_id, $title, $description, $author, $filename, $filesize, $asunto)
	{
		global $_user;
		...
		//CAMBIO FD - SI EL ASUNTO ESTA VACIO LE PONEMOS EL LITERAL (SIN ASUNTO)
		$this->asunto = ($asunto!='')?$asunto:'(Sin Asunto)';
		...
		else 
		{
			...
			// Esto es para que el titulo del correo no se nos quede nunca vacio
	...
	function _createExistingWork ($id)
	{
		...
		$sql="SELECT uploader_id, filename, filesize, title, description, author, upload_date, last_upload_date, cat_id,asunto
				FROM ".dropbox_cnf("tbl_file")."
		...
		$this->last_upload_date = $res["last_upload_date"];
		$this->category = $res['cat_id'];
		$this->asunto = $res['asunto'];
		
		//Para obtener la fecha del ultimo comentario en caso de que exista
		$sql = "SELECT max(feedback_date) as fecha FROM " .dropbox_cnf("tbl_feedback"). "WHERE file_id='".$id."'";
 		$result = api_sql_query($sql, __FILE__, __LINE__);
		$res = mysql_fetch_array($result,MYSQL_ASSOC);
		if ($res["fecha"] <> "")
		{
			$fecha_formateada = date("d-m-Y H:i:s",strtotime($res["fecha"]));
			$this->lastfeedback = $fecha_formateada;
		}
		else
		{
			$this->lastfeedback = "-";		
...
class Dropbox_SentWork extends Dropbox_Work
{
	 ...
	 * @return Dropbox_SentWork
		*/
	function Dropbox_SentWork ($arg1, $arg2=null, $arg3=null, $arg4=null, $arg5=null, $arg6=null, $arg7=null, $arg8=null)
	{
		if (func_num_args()>1)
		{
		    $this->_createNewSentWork ($arg1, $arg2, $arg3, $arg4, $arg5, $arg6, $arg7 ,$arg8);
		}
		else
		{
	 ...
	 * @param unknown_type $recipient_ids
		*/
	//añadimos el asunto
	function _createNewSentWork ($uploader_id, $title, $description, $author, $filename, $filesize, $recipient_ids, $asunto)
	{
		// Call constructor of Dropbox_Work object
		$this->Dropbox_Work($uploader_id, $title, $description, $author, $filename, $filesize, $asunto);
...
class Dropbox_Person
{
	...
	function Dropbox_Person ($userId, $isCourseAdmin, $isCourseTutor)
	{
		...
		$feedback_tbl = Database::get_course_table(TABLE_DROPBOX_FEEDBACK);
		...
		$sql = "SELECT r.file_id, r.cat_id
			...
			UNION
			    SELECT f.id,0
				FROM $file_tbl f, $person_tbl p, $feedback_tbl fk
