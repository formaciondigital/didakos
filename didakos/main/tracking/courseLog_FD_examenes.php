...
echo '<div class="clear"></div>';
	...
	if(count($a_students)>0)
	{
		...
		while ($alumno = Database :: fetch_array($a_students))
		{
			...
			if($export_csv )
			{
					   ...
					   if ($_GET["export"] == 'csv_intentos'){
							...
							$intentos_examenes = Tracking_fd ::get_intentos_examenes($course_code, $student_id);
							foreach($intentos_examenes as $intentos ){
		...
		$row[] = get_lang('NotaMediaDeExamenesRealizados').': ';
		...
		print_r($csv_content);*/
				if($export_csv)
			{
					   ...
					   if ($_GET["export"] == 'csv'){
					   $csv_content[0] = array ( get_lang('LastName'), get_lang('FirstName'), 'Notas Exámenes');
