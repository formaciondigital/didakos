...
{
	$_SESSION['selected_user'] = $_GET['user_id'];
	$_SESSION['tipo'] = $_GET['tipo'];
}
$a_students = Tracking_fd :: alumno_real_curso($_course['id'],$_SESSION['selected_user']);
...
function count_conections_in_course()
{
	global $nbConCourse;
	return $nbConCourse;
}

function count_conections_in_platform()
{
	global $nbConPlatform;
	return $nbConPlatform;
}
...
// ## FdTablaFin: Pinta el fin de tabla
// *****************************************************************************************
...
echo '<div class="clear"></div><br><br>';
	...
	//Hemos recibido un user_id. Mostramos la tabla normal con los datos del alumno.
	$student_id=$_SESSION['selected_user'];	
	$course_code = $_course['id'];								
	$student_datas = UserManager :: get_user_info_by_id($student_id);			
	$datos =  array ();
	$datos [0][0] = $student_datas['lastname'];
	$datos [0][1] = $student_datas['firstname'];
	$datos [0][2] = Tracking :: get_last_connection_date($student_id);
	$datos [0][3] = Tracking :: get_last_connection_date_on_the_course($student_id, $course_code);

	//Pintamos la tabla
	$columnas = array (get_lang('LastName'),get_lang('FirstName'),get_lang('UltimoAccesoPlataforma'),get_lang('UltimoAccesoCurso'));
	...
	echo FdTablaFila($datos);
	echo FdTablaFin();


	if ( isset ($_SESSION['tipo']))
	//Hemos recibido petición de expandir información de conexiones
	{
		switch ($_SESSION['tipo'])
		{
			case 'plataforma':
				echo '<br><br><b>'.get_lang('InformeExpandidoConexionesPlataforma').'</b><br><br>';
				$table = new SortableTable('tracking', 'count_conections_in_platform');		
				$table -> set_header(0, get_lang('FechaLogin'), false,'align="center"');
				$table -> set_header(1, get_lang('FechaLogout'), false,'align="center"');
				$all_datas = array();	
                
				$conexiones = Tracking_fd :: get_conexiones_plataforma($_SESSION['selected_user']);
				$nbConPlatform = Database::num_rows($conexiones);
				//iniciamos el array para el CSV
				$csv_content[] = array('','');
				$csv_content[0] =  array(get_lang('FechaLogin'),get_lang('FechaLogout'));

				foreach($conexiones as $conexion)
				{
					$row = array();
					$row[] = format_locale_date(get_lang('DateFormatLongWithoutDay'), strtotime($conexion['login_date'])) . ' ' . substr($conexion['login_date'],11) ;
					$row[] = format_locale_date(get_lang('DateFormatLongWithoutDay'), strtotime($conexion['logout_date'])) . ' ' . substr($conexion['logout_date'],11) ;
					$all_datas[] = $row;
					//añadimos al CSV
					$csv_content[] = $row;
				}
			...
			case 'curso':
				echo '<br><br><b>'.get_lang('InformeExpandidoConexionesPlataforma').'</b><br><br>';
				$table = new SortableTable('tracking', 'count_conections_in_course');		
				$table -> set_header(0, get_lang('FechaLogin'), false,'align="center"');
				$table -> set_header(1, get_lang('FechaLogout'), false,'align="center"');
				$all_datas = array();	
                
				$conexiones = Tracking_fd :: get_conexiones_curso($_course['id'], $_SESSION['selected_user']);
				
				
				$nbConCourse = count($conexiones);
				//iniciamos el array para el CSV
				$csv_content[] = array('','');
				$csv_content[0] =  array(get_lang('FechaLogin'),get_lang('FechaLogout'));
				foreach($conexiones as $conexion)
	...
	if($export_csv)
	{
		...
		//Formato XLS
		if ( $_SESSION['tipo']=='plataforma' )
		{
			Export_FD :: export_table_xls($csv_content, 'informe_seguimiento_plataforma_'.  $student_datas['official_code']);
		}
		else
		{
			if ($_SESSION['tipo']=='curso')
			{
				Export_FD :: export_table_xls($csv_content, 'informe_seguimiento_curso_'.  $student_datas['official_code'] . '_' . $_course['id']);
			}
		}
	}
