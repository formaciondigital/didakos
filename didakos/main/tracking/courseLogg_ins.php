...
//echo $nbStudents;
$a_students = Tracking_fd :: alumnos_reales_curso($_SESSION["id_curso"]);
...
//eliminado el enlace a las conexiones de alumnos
...
// else display student list with all the informations
	...
	*/
	...
	if(count($a_students)>0)
	{
		...
		//MENSAJES EN EL FORO
		$table -> set_header(2, 'Mensajes en el Foros',false);
		//MENSAJES EN EL BUZÓN
		//$table -> set_header(4, 'Mensajes en el Buzón',false);
		//MEDIA EN LOS EXÁMENES
		$table -> set_header(3, 'Media en los exámenes',false);
		//MEDIA EN EL SEGUIMIENTO
		//$table -> set_header(6, 'Progreso Medio de Seguimiento en Curso (%)',false);
		//MEDIA EN LOS EJERCICIOS
		$table -> set_header(4, 'Media en los ejercicios',false);		
		//PRIMER ACCESO AL CURSO
		$table -> set_header(5, get_lang('FirstLogin'), false, 'align="center"');
		//ÚLTIMO ACCESO AL CURSO
		$table -> set_header(6, get_lang('LatestLogin'), false, 'align="center"');
		//DETALLES
		...
		$csv_content[] = array('','','','','','','','','','');			
	    	...
	    	$all_datas = array();
	    	$course_code = $_SESSION["id_curso"];
		
		//Obtenemos el número total de exámenes para el curso tratado
		$total_examenes=Tracking_fd ::numero_total_examenes_curso($course_code,0);
		//Obtenemos el número total de ejercicios para el curso tratado
		$total_ejercicios=Tracking_fd ::numero_total_ejercicios_curso($course_code,1);
		...
		while ($alumno = Database :: fetch_array($a_students))
		{
			...
			//cambio fd - 07/03/2011 modificado para que redirija a la pagina del detalle del curos modificada para el inspector
