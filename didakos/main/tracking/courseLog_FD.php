...
//cambio fd - 07/03/2011 - si el usuario es inspector se redirige a su propia pagina de informes
if($_SESSION["_user"]["status"]==4){
$_SESSION["id_curso"] = $_course['id'];
header("Location:courseLogg_ins.php");
die();
}
...
*/
if($_GET['studentlist'] == 'true' || !isset($_GET['studentlist']))
{
	echo ' <a href="'.api_get_self().'?'.api_get_cidreq().'&export=csv"><img align="absbottom" src="../img/excel.gif">&nbsp;'.get_lang('Exportar').'</a>';
}
echo ' <a href="csv_informe_global.php?'.api_get_cidreq().'" target="_blank"><img align="absbottom" src="../img/excel.gif">&nbsp;'.get_lang('InformeGlobal').'</a>';
...
// else display student list with all the informations
	...
	//Comentado Formulario que muestra recordatorio de avisos de usuarios inactivos según número de días
	/*
	...
	*/
	...
	if(count($a_students)>0)
	{
		...
		$table -> set_header(2, get_lang('TiempoDePermanenciaEnElCurso'),false);
		//MENSAJES EN EL FORO
		$table -> set_header(3, get_lang('MensajesEnElForo'),false);
		//MENSAJES EN EL BUZÓN
		$table -> set_header(4, get_lang('MensajesEnElBuzon'),false);
		//MEDIA EN LOS EXÁMENES
		$table -> set_header(5, get_lang('MediaEnLosExamenes'),false);
		//MEDIA EN EL SEGUIMIENTO
		$table -> set_header(6, get_lang('ProgresoMedioDeSeguimientoEnCurso'),false);
		//MEDIA EN LOS EJERCICIOS
		$table -> set_header(7, get_lang('MediaEnLosEjercicios'),false);		
		...
		//Obtenemos el número total de scos para el curso tratado
		...
		while ($alumno = Database :: fetch_array($a_students))
		{
			...
			// Variable para mensajes en buzón
			$messages_in_course=0;

			// Variable para mensajes en buzón
			$messages_in_dropbox=0;
			
			// Variable para la media de los exámenes
			$media_examenes=0;
			...
			// Número de mensajes en foros				
			$messages_in_course = Tracking :: count_student_messages ($student_id, $course_code);
			// Número de mensajes en buzón								
			// $messages_in_dropbox = Tracking_fd ::count_dropbox_file ($student_id, $course_code);
			$messages_in_dropbox = Tracking_fd ::count_dmail ($student_id, $course_code);
			// Nota media de los exámenes para el alumno y curso tratado
			$media_examenes = Tracking_fd ::media_examenes_alumno_curso ($student_id, $course_code, $total_examenes);
				
			// Nota media de las notas de los últimos intentos de los ejercicios para el alumno y curso tratado
