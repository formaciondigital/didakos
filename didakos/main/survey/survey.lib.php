...
class SurveyUtil {
	...
	function display_complete_report()
	{
		...
		while ($row = Database::fetch_array($result))
		{
			...
			{
                                /* echo '<pre>';
                                   print_r($answers_of_user);
                                   echo '</pre>';

                                     echo '<pre>';
                                   print_r($possible_answers);
                                   echo '</pre>';
				...
				if(is_array($possible_option) && count($possible_option)>0)
				{
					foreach ($possible_option as $option_id=>$value)
					{
						...
						elseif (!empty($answers_of_user[$question_id][$option_id]))
						{
                                                    ...
                                                    /*
                                                     * 30/03/2011 - CAMBIO FD
                                                     * SE AÑADE ESTA COMPROBACION PARA SOLUCIONAR UN PROBLEMA QUE SE PRODUCIA AL EXPORTAR EL INFORME A
                                                     * EXCEL,NO MOSTRABA LA PUNTUACION NUMERICA EN PREGUNTAS TIPOS 'SCORE',EN SU LUGAR MOSTRABA UNA 'V'
                                                     */
