	...
	switch ($type)
	{
		...
		case TOOL_QUIZ:
			if(!empty($id))
			{
				...
				//Si es un examen (en lugar de un ejercicio), se redirige a la página correspondiente de control del examen
				$destino="exercice";				
				$TBL_EXAMS = Database::get_course_table(TABLE_QUIZ_TEST_EXAM,$_course['database']);
				$sql_ex = "SELECT * FROM $TBL_EXAMS WHERE id=$id";
				$result_ex= api_sql_query($sql_ex,__FILE__,__LINE__);
				if (Database::num_rows($result_ex)>0)    {
					$destino="exam";
				}							
				...
				$link .= $main_dir_path.$destino.'/exercice_submit.php?lp_init=1&origin='.$origin.'&learnpath_id='.$learnpath_id.'&learnpath_item_id='.$id_in_path.'&exerciseId='.$id;
			}
			break;
