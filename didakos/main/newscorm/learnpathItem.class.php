...
class learnpathItem{
    ...
    function get_score(){
    	...
    	{
 			if($this->current_score!="undefined"){
      	...
      	//check the row exists
	     	...
	     	if(Database::num_rows($check_res)<1){
				...
				$sql = "INSERT INTO $item_view_table " .
						...
						//FD: SI  ENTRAMOS EN UN TIPO SCO PONEMOS EL TIEMPO A 0, YA SE MODIFICA EN EL FINISH
						if($this->type=='sco'){
							$sql = "INSERT INTO $item_view_table " .
								"(total_time, " .
								"start_time, " .
				...
				{
					//FD DIFERENCIAMOS SI VENIMOS DE UN SCO PARA NO GUARDAR EL TIEMPO , SÓLO DEJAMOS SU VALOR
					if($this->type=='sco'){
					     	$sql = "UPDATE $item_view_table " .
...
//		     					"AND order_id = $index";
	  ...
	  /**
     * NUEVA FUNCION FD SEGUIMIENTO PARA GUARDAR EL TIEMPO
	 * GUARDAMOS EL TIEMPO EN LA BASE DE DATOS
     * @return	boolean	Query result
     */
     function guardar_tiempo_sco_db()
     {
   		if($this->debug>0){error_log('New LP - In learnpathItem::write_to_db()',0);}
   		$mode = $this->get_lesson_mode();
   		$credit = $this->get_credit();
      	...
      	//check the row exists
	    	...
	    	$sql = '';
	     	if($this->type=='sco')
	     	{	
				//OBTENEMOS EL TIEMPO TOTAL ACUMULADO EN NUESTRO SCO
				$sql_tiempo = "SELECT total_time FROM $item_view_table " .
					"WHERE lp_item_id = ".$this->db_id." " .
	     			"AND   lp_view_id = ".$this->view_id." " .
	     			"AND   view_count = ".$this->attempt_id;
				
				$res = api_sql_query($sql_tiempo,__FILE__,__LINE__);
				$tiempo_acumulado=0;
				if(Database::num_rows($res)>0){
					$row = Database::fetch_array($res);
					$tiempo_acumulado = $row[0];
				}
				
				$tiempo_acumulado+=$this->get_total_time();
				
				//HACEMOS UN UPDATE DEL TIEMPO  EN NUESTRO SCO
			    $sql = "UPDATE $item_view_table " .
							"SET total_time = ".$tiempo_acumulado." " .
			     			"WHERE lp_item_id = ".$this->db_id." " .
			     			"AND lp_view_id = ".$this->view_id." " .
			     			"AND view_count = ".$this->attempt_id;
