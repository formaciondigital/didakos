...
function LMSGetValue(param) {
	...
	// ---- cmi.core.student_id
		...
		//--firstName='<?php echo $_user['firstName']; ?>'
		<?php
		//FD OBTENEMOS EL ARRAY USER INFO PARA SACAR EL USERNAME
		$uinfo = api_get_user_info(api_get_user_id());
		$pos = strpos(strtolower($uinfo['username']), 'demo');
		if ($pos === false) {
		?>
			result='<?php echo $_user['user_id']; ?>';
		<?php
		...
		?>
			result='<?php echo 'demo'.$_user['user_id']; ?>';
		<?php
		}
		?>
...
function savedata(origin) { //origin can be 'commit', 'finish' or 'terminate'
	if ((lesson_status != 'completed') && (lesson_status != 'passed') 
    	&& (mastery_score >=0) && (parseInt(score) >= parseInt(mastery_score)))
    {
		//alert ("score-->" +score + "mastery_score " + mastery_score);
...
function LMSFinish(val) {
	//FD: HACEMOS LA LLAMADA A LA FUNCION QUE ESTA DENTRO DE LP_COMM.SERVER.PHP QUE A SU VEZ LLAMARÁ A LA DEL LEARNPATHITEM PARA GUARDAR EL TIEMPO ACUMULADO EN NUESTRO SCO
	xajax_guardar_tiempo_sco(lms_lp_id, lms_user_id, lms_view_id, lms_item_id);
...
function XAJAXobject() {
  ...
  // FD: AÑADIMOS LA FUNCION NUEVA QUE HEMOS CREADO PARA GUARDAR EL TIEMPO DEL SCO
  this.xajax_save_item = xajax_guardar_tiempo_sco;
}
...
/************************************************************************************************************
	VARIABLE CREADA PARA LA	MODIFICACIÓN FORMACIÓN DIGITAL 20 JULIO 2011 (RETARDO 5 SEGUNDOS DE CARGA SCO)
************************************************************************************************************/
var intervalo=null;
function switch_item(current_item, next_item){
	...
	**************************/
	...
	/***********************************************************************************
		MODIFICACIÓN FORMACIÓN DIGITAL 20 JULIO 2011 (RETARDO 5 SEGUNDOS DE CARGA SCO)
	************************************************************************************/
	var mysrc = 'lp_controller.php?action=content&lp_id='+lms_lp_id+'&item_id='+next_item;
	var cont_f = document.getElementById('content_id');
	if(!cont_f){
		logit_lms('In switch - content frame not found',0);
		<?php if($oLP->mode = 'fullscreen'){ ?>
		cont_f=window.open('http://gateway.formaciondigital.com/cargando/lms.htm','content_name','toolbar=0,location=0,status=0,scrollbars=1,resizable=1');
		clearInterval(intervalo);
		intervalo = setTimeout("window.open('"+mysrc+"','content_name','toolbar=0,location=0,status=0,scrollbars=1,resizable=1')",5000);
		<?php } else { ?>
			return false;
		<?php } ?>
	}
	...
	{
		cont_f.src ='http://gateway.formaciondigital.com/cargando/lms.htm';
		clearInterval(intervalo);
		intervalo=setTimeout("document.getElementById('content_id').src = '"+mysrc+"';",5000);
	}
