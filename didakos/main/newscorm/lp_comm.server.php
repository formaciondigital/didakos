...
function backup_item_details($lp_id,$user_id,$view_id,$item_id,$score=-1,$max=-1,$min=-1,$status='',$time='',$suspend='',$location='')
{
	...
	//INICIO MODIFICACIÓN SCORM FD (PONEMOS POR CADA LINEA UN $objResponse->addScript)
		$objResponse->addScript("old_score=".$score.";");
		$objResponse->addScript("old_max=".$max.";" );
		$objResponse->addScript("old_min=".$min.";");
		$objResponse->addScript("old_lesson_status='".$status."';");
		$objResponse->addScript("old_session_time='".$time."';");
		$objResponse->addScript("lms_old_item_id='".$item_id."';");
		$objResponse->addScript("old_suspend_data='".$suspend."';");		
		$objResponse->addScript("old_lesson_location='".$location."';");
	//FIN  MODIFICACIÓN SCORM	FD	
 ...
 */
function save_item($lp_id,$user_id,$view_id,$item_id,$score=-1,$max=-1,$min=-1,$status='',$time=0,$suspend='',$location='',$interactions=array(),$core_exit='none',$tiempoSCO)
{
...
function switch_item_details($lp_id,$user_id,$view_id,$current_item,$next_item)
{
	...
	switch($next_item){
		...
		default:
			...
			if($next_item == $current_item){
				...
				//PARA QUE SE GUARDEN CORRECTAMENTE LOS VALORES DEL SEGUIMIENTO COMENTAMOS LA LINEA DE REINICIAR EL ELEMENTO PULSADO
	...
	*/
	...
	//INICIO MODIFICACIÓN SCORM FD (PONEMOS POR CADA LINEA UN $objResponse->addScript)
		$objResponse->addScript("score=".$myscore.";");
		$objResponse->addScript("max=".$mymax.";");
		$objResponse->addScript("min=".$mymin.";");
		$objResponse->addScript("lesson_status='".$mylesson_status."';");
		$objResponse->addScript("lesson_location='".$mylesson_location."';");
		$objResponse->addScript("session_time='".$mysession_time."';");
		$objResponse->addScript("suspend_data='".$mysuspend_data."';");
		$objResponse->addScript("lesson_location='".$mylesson_location."';");
		$objResponse->addScript("total_time = '".$mytotal_time."';");
		$objResponse->addScript("mastery_score = '".$mymastery_score."';");
		$objResponse->addScript("max_time_allowed = '".$mymax_time_allowed."';");
		$objResponse->addScript("launch_data = '".$mylaunch_data."';");
		$objResponse->addScript("interactions = new Array();");
		$objResponse->addScript("item_objectives = new Array();");
		$objResponse->addScript("G_lastError = 0;");
		$objResponse->addScript("G_LastErrorMessage = 'No error';");
	//FIN  MODIFICACIÓN SCORM	FD
	...
	//INICIO MODIFICACIÓN SCORM FD (PONEMOS POR CADA LINEA UN $objResponse->addScript)
		$objResponse->addScript("saved_lesson_status='not attempted';");
		$objResponse->addScript("lms_lp_id=".$lp_id.";");
		$objResponse->addScript("lms_item_id=".$new_item_id.";");
		$objResponse->addScript("lms_old_item_id=0;");
		$objResponse->addScript("lms_been_synchronized=0;");
		$objResponse->addScript("lms_initialized=0;");
		$objResponse->addScript("lms_total_lessons=".$mytotal.";");
		$objResponse->addScript("lms_complete_lessons=".$mycomplete.";");
		$objResponse->addScript("lms_progress_bar_mod='".$myprogress_mode."';");
		$objResponse->addScript("lms_view_id=".$view_id.";");
		$objResponse->addScript("lms_user_id=".$user_id.";");
		$objResponse->addScript("next_item=".$new_item_id.";"); //this one is very important to replace possible literal strings
		$objResponse->addScript("lms_next_item=".$mynext.";");
		$objResponse->addScript("lms_previous_item=".$myprevious.";");
		$objResponse->addScript("lms_item_type = '".$myitemtype."';");
		$objResponse->addScript("lms_item_credit = '".$mycredit."';");
		$objResponse->addScript("lms_item_lesson_mode = '".$mylesson_mode."';");
		$objResponse->addScript("lms_item_launch_data = '".$mylaunch_data."';");
		$objResponse->addScript("lms_item_interactions_count = '".$myinteractions_count."';");
		$objResponse->addScript("lms_item_objectives_count = '".$myinteractions_count."';");
		$objResponse->addScript("lms_item_core_exit = '".$mycore_exit."';");
		$objResponse->addScript("asset_timer = 0;");
	//FIN  MODIFICACIÓN SCORM FD
...
/**
 * TRAS HACER EL LMS_FINISH GUARDAMOS EL TIEMPO EMPLEADO EN EL SCO. NUEVA FUNCION DE FD PARA GUARDAR CORRECTAMENTE EL TIEMPO DEL SCO
  */
function guardar_tiempo_sco($lp_id,$user_id,$view_id,$item_id){
	$objResponse = new xajaxResponse();
	require_once('learnpath.class.php');
	require_once('scorm.class.php');
	require_once('aicc.class.php');
	...
	//LLAMAMOS A LA FUNCIÓN DENTRO DEL LEARNPATHITEM.CLASS.PHP QUE ACTUALIZA EL TIEMPO DE NUESTRO SCO
	$mylpi->guardar_tiempo_sco_db();
	return $objResponse;
}
