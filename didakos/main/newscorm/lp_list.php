...
Página incial: lp_list.php (1.8.5)
Página actual: lp_list.php
Descripción: Página modificada para que los tutores no tengan acceso a las herramientas de
modificación de los itinerarios formativos. Se ha hecho añadiendo "&& api_is_platform_admin()" a 
las zonas en las que carga las herramientas de administración. Actualmente solo un administrador o
Gestor tienen acceso a estas herramientas. Para ver cambios buscar "##cambio"
22/01/09 egarcia - eliminamos también el acceso del gestor a ciertas opciones. El el ultimo cambio de la pagina.
...
if (is_array($flat_list))
{
	...
	foreach ($flat_list as $id => $details)
	{
		...
		// ##cambio
	    if($is_allowed_to_edit && api_is_platform_admin())
	    {
	...
	##cambio
	if($is_allowed_to_edit && api_is_platform_admin() && $_SESSION["_user"]["user_id"] != 1 )
	{
		   echo $dsp_line.$dsp_progress.$dsp_desc.'<td align="center">&nbsp;</td>'.$dsp_edit.$dsp_build.$dsp_visible.$dsp_publish.'<td></td>';
	}
	else
	{
