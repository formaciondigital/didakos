...
Página incial: user.php (1.8.5)
Página actual: user.php
Descripción: Página modificada para que los tutores no tengan acceso a las herramientas de
modificación de los alumnos. Se ha hecho añadiendo "&& api_is_platform_admin()" a 
las zonas en las que carga las herramientas de modificación. Actualmente solo un administrador o
Gestor tienen acceso a estas herramientas. Para ver cambios buscar "##cambio"
 ...
 if( api_is_allowed_to_edit())
{
	...
	//##cambio, solo el admin... ya ni el gestor.
	if( api_is_platform_admin() && $_SESSION["_user"]["user_id"] == 1)
	{
...
function get_user_data($from, $number_of_items, $column, $direction)
{
	...
	if(!empty($_SESSION["id_session"])){
		...
		$a_course_users = CourseManagerFD :: get_user_list_from_course_code_fd($_SESSION['_course']['id'], true, $_SESSION['id_session'], $limit, $order_by);
	}
	else
	{		
		$a_course_users = CourseManagerFD :: get_user_list_from_course_code_fd($_SESSION['_course']['id'], true, 0, $limit, $order_by);
	}
	...
	foreach($a_course_users as $user_id=>$o_course_user)
	{
		...
		//Vamos a eliminar aqui del listado a administrador, gestor y a aquellos demo que tenga firstname='DEMO'
		if ($user_id!=1 && $user_id!=2 && $o_course_user['firstname']!='DEMO')
		{
			if(api_is_allowed_to_edit())
			{
				...
				$temp[] = $o_course_user['email'];	
					
				// Remodelamos todo este apartado para hacerlo público a los alumnos
				// deja de mostrarse el username al no ser útiles para los alumnos y el
				// password por cuestiones obvias de provacidad.
...
function modify_filter($user_id)
{
	...
	/*
	egarcia 20/10/2010 eliminamos este acceso. Permite modificar el rol del alumno a administrador.
	if(!api_is_anonymous())
	{
	...
	*/
	...
	{
		$result .= '<a href="../mySpace/myStudents.php?'.api_get_cidreq().'&details=true&student='.$user_id.'&amp;course='.$_course['id'].'&amp;origin=user_course" title="'.get_lang('Tracking').'"  ><img border="0" alt="'.get_lang('Tracking').'" src="../img/statistics.gif" /></a>&nbsp;';
	}
	// ##cambio ... Se excluye también al gestor de eliminar alumnos
	/*
	if(api_is_allowed_to_edit() )
	{
		if( $user_id != $_user['user_id'] && $_SESSION["_user"]["user_id"] == 1)
		{
		// edit
	...
	*/
	...
	// Añadimos el enlace al perfil del alumno
	$result .= '<a href="perfil.php?'.api_get_cidreq().'&student='.$user_id.'"  ><img border="0" alt="'.get_lang('User_info').'" src="../img/user_info.gif" /></a>&nbsp;';

	// Ahora obtenemos el Follow de twitter en caso de que disponaga de perfil.
	...
	from  ' .$tbl_user_field. ' uf LEFT OUTER JOIN ' . $tbl_user_field_values . ' ufv on (uf.id =ufv.field_id) where ufv.user_id=' . $user_id .
	' and uf.id=10';
	$res = api_sql_query($sql,__FILE__,__LINE__);
	$row = Database::fetch_array($res);
	
	$twitter_user = substr (  $row['field_value'],  strrpos($row['field_value'], "/") +1);
	if ($twitter_user!="")
	{
		$result .= '<a href="http://twitter.com/' . $twitter_user . '" class="twitter-follow-button" data-show-count="false" data-lang="es">Follow @' .$twitter_user.'</a>
...
$table->set_header($header_nr++, get_lang('email'),false);
...
//##cambio
 if( api_is_allowed_to_edit() && api_is_platform_admin() && $_SESSION["_user"]["user_id"] == 1 )
{
