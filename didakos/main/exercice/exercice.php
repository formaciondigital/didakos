...
// Se excluyen los exámenes del listado de ejercicios
//$sql="SELECT count(id) FROM $TBL_EXERCICES";
$sql="SELECT count(a.id) FROM $TBL_EXERCICES a WHERE a.id not in (select b.id from $TBL_EXAMS b)";
...
if($is_allowedToEdit)
{
	...
	{
		// Se excluyen los exámenes del listado de ejercicios
		$sql="SELECT a.id,a.title,a.type,a.active,a.description, a.results_disabled FROM $TBL_EXERCICES a WHERE a.active<>'-1' and a.id not in (select b.id from $TBL_EXAMS b) ORDER BY a.title LIMIT $from,".($limitExPage+1);
		$result=api_sql_query($sql,__FILE__,__LINE__);
	}
}
// only for students
...
{
	// Se excluyen los exámenes del listado de ejercicios
	$sql="SELECT a.id,a.title,a.type,a.description, a.results_disabled FROM $TBL_EXERCICES a WHERE a.active='1' and a.id not in (select b.id from $TBL_EXAMS b) ORDER BY a.title LIMIT $from,".($limitExPage+1);
	$result=api_sql_query($sql,__FILE__,__LINE__);
}
...
if($show == 'test'){
	...
	if (($is_allowedToEdit) and ($origin != 'learnpath'))
	{
		echo '<td width="50%" nowrap="nowrap">'.
			...
			' | <img src="'.api_get_path(WEB_IMG_PATH).'show_test_results.gif" align="absbottom">&nbsp;<a href="'.api_add_url_param($_SERVER['REQUEST_URI'],'show=result').'">'.get_lang("Results").' </a></p>'.
			'</td>'.
			'<td width="50%" align="right">';
			// egarcia 06/07/11 Hemos añadido aquí la opción "Resultados" que muestra las notas de los alumnos en los ejercicios
	}
	else
	{
		echo '<td align="right">';
	...
	if($show == 'result')
	{
		...
		if($is_allowedToEdit || $is_tutor)
		{
			...
			$sql="SELECT CONCAT(lastname,' ',firstname),ce.title, te.exe_result ,
				  ...
				  WHERE te.exe_exo_id = ce.id AND user_id=te.exe_user_id AND te.exe_cours_id='$_cid' and ce.active=1
				  ORDER BY te.exe_cours_id ASC, ce.title ASC, te.exe_date ASC";
		// Añadimos "and ce.active=1" para que solo muestre ejercicios activos.
		// egarcia 06/07/11
		...
		//QUITAMOS EL ACCESO A LOS RESULTADOS DE LOS EJERCICIOS
