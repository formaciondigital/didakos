...
$num_mensajes_buzon = Tracking_fd ::count_dmail($id_usuario,$id_course);
...
<script language="javascript">
	function compruebaNota(){			
		...
		if (isNaN(nota) || (nota.indexOf(",")>= 0) || (nota.indexOf(".")>= 0) || (nota<0) || (nota>10)){
			...
			alert("<?php echo get_lang('DebeIntroducirValor'); ?>");						
...
<tr class='<?php echo $i?'row_odd':'row_even'; ?>'>
	...
	<td align='right'>
<?php
//cambio fd - 27-10-2010 - evitamos que el perfil de recuross humanos pueda cambiar la nota de un alumno
		if ($profesor==1 && $_user['status']!= DRH ) // el profesor puede modificar la nota del alumno
		{
?>	
			...
			<input type="button" value="<?php echo get_lang('langOk'); ?>"  onclick="javascript:compruebaNota();"> 	
<?php
		}
...
$csv_content[] = array("");
$csv_content[] = array(get_lang('EXAMENES'));
$csv_content[] = array(get_lang('langScormContentColumn'),get_lang('Nota'));
	...
	//##########  CSV	 ##########
			...
			if ($profesor==1){
					...
					if (is_numeric($notaExamen)) {
						...
						echo "<a href='../exam/exercise_show_fd.php?origin=tracking_course&id=".$exe_id."&cidReq=".$id_course."&student=".$id_usuario."'>".get_lang('Ver_examen')."</a>";
					}		
					echo "&nbsp;</td>";	
			}
...
//##########  CSV	 ##########
$csv_content[] = array(get_lang('MoyenneExamen'),$media);
//Antiguo
...
<table class="data_table" border='1' width='100%'>
	...
	<tr><th><?php echo get_lang('langWorkTitle'); ?></th><th><?php echo get_lang('StartDate'); ?></th><th><?php echo get_lang('langScormStatusColumn'); ?></th><th><?php echo get_lang('Progress'); ?>(%)</th><th><?php echo get_lang('Time'); ?></th></tr>
<?php
$time_for_total = 0;
$score_media    = '-';
...
?>
	<tr><td><?php echo get_lang('SynthesisView'); ?></td><td>&nbsp;</td><td>&nbsp;</td><td><?php echo $score_media ?></td><td><?php echo $time_for_total ?></td></tr>
</table>
...
//##########  CSV	 ##########
 $csv_content[] = array("");
 $csv_content[] = array(get_lang('EJERCICIOSCOMPLEMENTARIOS')); //array("EJERCICIOS COMPLEMENTARIOS");
 $csv_content[] = array(get_lang('langScormContentColumn'),get_lang('NotaUltimoIntento'),get_lang('NumeroIntentos')); //array("Título","Nota Último Intento","Número Intentos");
