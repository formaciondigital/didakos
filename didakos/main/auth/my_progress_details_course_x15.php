...
//Incluimos librerÃ­a de exportaciÃ³n a CSV, XLS,... de FD
require_once (api_get_path(LIBRARY_PATH).'export.lib.inc_FD.php');

//Incluimos librerÃ­a de utilidades generales
...
if (!isset($_GET['student'])) //Estamos en pÃ¡gina para el alumno
{
	$id_usuario = $_user['user_id'];
	$parametros = "?course=$id_course";
}
else //Estamos en pÃ¡gina para el profesor o el admin
{
...
// ActualizaciÃ³n de la nota global por parte del teletutor
...
<script language="javascript">
	function confirmar_eliminar_seguimiento(usuario, curso, seguimiento)
	{		
		if (confirm("¿Está seguro de eliminar este seguimiento?"))
		{
				location.href="eliminar_seguimiento_x15.php?student="+usuario+"&course="+curso+"&seguimiento="+seguimiento;
		}			
	}
	...
	function compruebaNota(){			
		...
		if (isNaN(nota) || (nota.indexOf(",")>= 0) || (nota.indexOf(".")>= 0) || (nota<0) || (nota>10)){
			...
			alert("<?php echo get_lang('Debe introducir un valor numÃ©rico entero comprendido entre 0 y 10'); ?>");						
...
<div align="right">
	<a href="<?php echo api_get_self().'?'.$_SERVER['QUERY_STRING'] ?>&export=csv"><img align="absbottom" src="../img/excel.gif">&nbsp;<?php echo get_lang('Export'); ?></a><br />&nbsp;<br />
</div>
<table class="data_table" width="100%">
<tr class="tableName"><td colspan="6"><strong><?php echo get_lang('GENERAL'); ?></strong></td></tr>
<tr class="tableName">
	<th><?php echo get_lang('TiempoTotal'); ?></th> 
	<th><?php echo get_lang('MensajesForo'); ?></th> 
	<th><?php echo get_lang('MensajesBuzon'); ?></th> 
...
//##########  CSV	 ##########
	$csv_content[] = array("");
	$csv_content[] = array("EXÃMENES");
	$csv_content[] = array("TÃ­tulo","Nota");
...
$sqlExercices = "SELECT q.title,q.id 
									 FROM ".$a_infosCours['db_name'].".".$tbl_course_quiz." q, ".$a_infosCours['db_name'].".".$tbl_course_lp_item." lp 
									WHERE q.id=lp.path and lp.item_type='quiz' and q.active='0' 
									order by lp.display_order";	

$resultExercices = api_sql_query($sqlExercices);

//Guardamos el nÃºmero de exÃ¡menes
...
if($numExamen>0){
	while($a_exercices = Database::fetch_array($resultExercices)){
			//Hacemos la consulta que saca la nota y la ponderaciÃ³n del examen (sÃ³lo se puede realizar UNA VEZ cada examen) que se haya realizado para el id de ejercicio sacado de la tabla quiz.	
			...
			//Mostramos el nombre del tÃ­tulo del examen		
	...
	//##########  CSV	 ##########
			...
			if ($profesor==1){
					...
					if (is_numeric($notaExamen)) {
						// mostramos detalle del examen, las preguntas  que le salieron al alumno y las respuestas que seleccionÃ³		
						// ejemplo de llamada
...
//##########  CSV	 ##########
$csv_content[] = array("Media de los ExÃ¡menes",$media);
?>
...
$sql = "SELECT iv.status as estado,  iv.score as progreso, iv.total_time as tiempo_sco, i.id as id_elemento, 
				 ...
				 WHERE i.id = iv.lp_item_id AND iv.lp_view_id = v.id  AND v.user_id =" .$id_usuario."  AND i.item_type='sco' 
...
//##########  CSV	 ##########
	...
	$csv_content[] = array("Tï¿œtulo","Fecha Inicio","Estado","Progreso(%)","Tiempo");
*/
?>	
<p></p>
<p></p>
<table class="data_table" border='1' width='100%'>
	<tr class="tableName">	<td colspan=7><strong><?php echo strtoupper(get_lang('CourseTracking')); ?></strong></td></tr>
	<tr><th><?php echo get_lang('langWorkTitle'); ?></th><th><?php echo get_lang('StartDate'); ?></th><th><?php echo get_lang('langScormStatusColumn'); ?></th><th><?php echo get_lang('Progress'); ?>(%)</th><th><?php echo get_lang('Time'); ?></th><th>Editar</th><th>Borrar</th></tr>
<?php
$time_for_total = 0;
$score_media    = '-';
...
if ($num_scos>0) {		
	while ($row = Database :: fetch_array($result)) {
		...
		echo "<tr><td>".$title."</td><td>".$start_time."</td><td>".$lesson_status."</td><td>".$score."%</td><td>".$time."</td><td><a href=editar_seguimiento_x15.php?student=".$id_usuario."&course=".$id_course."&seguimiento=".$scoIdentifier.">Editar</a></td><td><a href=\"javascript:confirmar_eliminar_seguimiento(".$id_usuario.",'".$id_course."',".$scoIdentifier.")\">Borrar</a></td></tr>";
...
// el alumno no ha entrado aÃºn en el itinerario de aprendizaje, no tiene filas en la tabla LP_ITEM_VIEW, por eso usamos la tabla LP_ITEM
	$sql ="SELECT i.id as id_elemento, i.title as titulo_sco,  i.item_type as tipo_elemento  
					...
					WHERE i.item_type='sco' 
					ORDER BY id_elemento";
	
	$result = api_sql_query($sql, __FILE__, __LINE__);
	while ($row = Database :: fetch_array($result)) {
			$title = $row['titulo_sco'];
			echo "<tr><td>".$title."</td><td>".$start_time."</td><td>".$lesson_status."</td><td>".$score."</td><td>".$time."</td><td>-</td><td>-</td></tr>";	
...
?>
	<tr><td><?php echo get_lang('SynthesisView'); ?></td><td>&nbsp;</td><td>&nbsp;</td><td><?php echo $score_media ?></td><td><?php echo $time_for_total ?></td><td colspan=2>&nbsp;</td></tr>
</table>
<p></p>
...
//##########  CSV	 ##########
	...
	$csv_content[] = array("TÃ­tulo","Nota Ãltimo Intento","NÃºmero Intentos");

//EJERCICIOS: 
//HACEMOS LA SELECT DE LA TABLA QUIZ PARA OBTENER LOS EJERCICIOS QUE NO PERTENECEN AL ITINERARIO DE APRENDIZAJE (ACTIVE=1 OJO ACTIVO, APARECEN SÃLO EN APARTADO EJERCICIOS)

$sqlExercices    = "SELECT title,id FROM ".$a_infosCours['db_name'].".".$tbl_course_quiz." WHERE active='1' order by id";	
$resultExercices = api_sql_query($sqlExercices);

//Guardamos el nÃºmero de ejercicios
...
if($numEjercicios>0){
	...
	//Hacemos la consulta que saca la nota y la ponderaciÃ³n del Ãºltimo ejercicio que se haya realizado para el id de ejercicio sacado de la tabla quiz.
	$sqlScore      = "SELECT exe_id , exe_result,exe_weighting FROM $tbl_stats_exercices 
										 WHERE exe_user_id = ".$id_usuario." AND exe_cours_id = '".$id_course."' AND exe_exo_id = ".$a_exercices['id']." 
										 ORDER BY exe_date DESC LIMIT 1";
	$resultScore   = api_sql_query($sqlScore);	
	...
	//Obtenemos el nÃºmero de intentos para cada ejercicio		
...
//##########  CSV	 ##########
	$csv_content[] = array("Media de los Ejercicios",$media,"");
?>
