...
$sql="SELECT count(a.id) FROM $TBL_EXERCICES a, $TBL_EXAMS b  WHERE a.id=b.id";
...
*/
	...
	if($show == 'test')
	{
		$sql="SELECT a.id,a.title,a.type,a.active,a.description, a.results_disabled FROM $TBL_EXERCICES a, $TBL_EXAMS b WHERE a.active<>'-1' and a.id=b.id ORDER BY a.id LIMIT $from,".($limitExPage+1);
		$result=api_sql_query($sql,__FILE__,__LINE__);
	}
}
// only for students
elseif($show == 'test')
{
	$sql="SELECT a.id,a.title,a.type,a.description, a.results_disabled FROM $TBL_EXERCICES a, $TBL_EXAMS b WHERE a.active='1' and a.id=b.id ORDER BY a.id LIMIT $from,".($limitExPage+1);
	$result=api_sql_query($sql,__FILE__,__LINE__);
}
