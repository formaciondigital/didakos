...
$language_file = array('exercice','exam');
...
//tabla que contiene los ejercicios que son exámenes	
$TBL_EXAMS             = Database::get_course_table(TABLE_QUIZ_TEST_EXAM);

$TBL_QUESTIONS_EXAMS   = Database::get_course_table(TABLE_QUIZ_TEST_QUESTION);
...
// FD: INICIO COMPROBACION SI HA REALIZADO YA EL EXAMEN PARA NO PERMITIR QUE LO HAGA DE NUEVO
$user_id=$_SESSION['_user']['user_id'];
$course=$_SESSION['_course']['id'];
...
$msg_error_ex="";
$num_preg=0;
$nameTools=get_lang('toolExams');

if (!empty ( $exercise_id ) ) {

	//Comprobar que es un examen (no un ejercicio)
	$sqlExam = "SELECT b.id, b.intentos, b.minimo, a.random FROM ".$TBL_EXERCICES." as a, ".$TBL_EXAMS." as b WHERE a.id=b.id and b.id = ".$exercise_id;
	$resultExam = api_sql_query($sqlExam);		

	if ($row = Database :: fetch_array($resultExam)) {	
		$intentos=$row['intentos'];
		$minimo=$row['minimo'];
		$random=$row['random'];
		
		$sqlMin = "SELECT question_id FROM ".$TBL_QUESTIONS_EXAMS." WHERE exercice_id = ".$exercise_id;
		$resultMin = api_sql_query($sqlMin);
		//número de preguntas del examen
		$num_preg=Database::num_rows($resultMin);
		if ($num_preg<$minimo) {		
			$error_ex = 1;
			$msg_error_ex = get_lang('ExamMin');			
		}else {
			if ($random<=$minimo) {				
				$sqlExercices = "SELECT exe_id FROM ".$TBL_TRACK_EXERCICES." WHERE exe_user_id = ".$user_id." AND exe_cours_id = '".$course."' AND exe_exo_id = ".$exercise_id;
				$resultExercices = api_sql_query($sqlExercices);
				//Guardamos el número de exámenes hechos por el usuario
				$numIntentoExamen=Database::num_rows($resultExercices);		
				if($numIntentoExamen >= $intentos){
					$error_ex = 1;
					$msg_error_ex = get_lang('ExamCompleted')." (".$intentos.").";
				}
			}else {
				$error_ex = 1;
				$msg_error_ex = get_lang('ExamRandomMin');
			}
		}
	}	
}
...
if($error_ex > 0){  // Error en alguna condición del examen
	...
	//Comprobar salida de error formateada con estilos de página desde las distintas páginas que invocan a ésta.
	...
	Display::display_error_message($msg_error_ex,false);
...
// FD: FIN COMPROBACION EXAMEN
	...
	$interbreadcrumb[]=array("url" => "exercice.php","name" => get_lang('toolExams'));
