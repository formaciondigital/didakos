...
/*	http://version03.formaciondigital.com/main/auth/my_progress_details_course.php?
student=5
&details=true
&course=10282ED1
...
//cambio fd - si el usuario que esta viendo el informe es inspector se comporta como si fuese un tutor
//$user_restriction = is_allowedToEdit  ? '' :  "AND user_id=".intval($_user['user_id'])." ";		
$user_restriction = ($is_allowedToEdit || $_user['status']==DRH) ? '' :  "AND user_id=".intval($_user['user_id'])." ";
  ...
  <?php
	...
	<?php }
		...
		<table width="355" border="0">
			...
			<?php
			$objAnswerTmp=new Answer($questionId);
			$objAnswerTmp->readOrderedBy("ID","ASC");
			$nbrAnswers=$objAnswerTmp->selectNbrAnswers();
//Depuración FD		
//echo "<p>nbrAnswers ".	$nbrAnswers;													
			$questionScore=0;
			
			for($answerId=1;$answerId <= $nbrAnswers;$answerId++)
				{
				$answer=$objAnswerTmp->selectAnswer($answerId); //mirar
				$answerComment=$objAnswerTmp->selectComment($answerId);
				$answerCorrect=$objAnswerTmp->isCorrect($answerId); //mirar
				$answerWeighting=$objAnswerTmp->selectWeighting($answerId); //mirar
...
//echo "--------------------------";				
//echo "<p>".	$queryans;							
//echo "<p>resultans ".	$resultans;							
//echo "<p>answerId ".	$answerId;										
//echo "<p>answer ".	$answer;										
//echo "<p>answerCorrect ".	$answerCorrect;										
//echo "<p>answerWeighting ".	$answerWeighting;										
//echo "<p>choice, selección alumno ".	$choice;		
//echo "<p>studentChoice - pinta_alumno ".	$studentChoice;											
//echo "<p>answerCorrect - pinta_correcta ". $answerCorrect;			
//echo "<p>questionScore ". $questionScore;											
//echo "<p>totalScore ". $totalScore;																	
					?>
...
?>
	...
	?>	
		...
		<!-- Modificación FD,  no se permite introducir retroalimentación a las preguntas, se ha eliminado el código que presenta el editor
       		 para introducir la observación del teletutor   -->												
				&nbsp;
		</td>
	</tr>
  <tr>
...
// Modificación FD, se muestra la corrección del examen, no se deja que el teletutor introduzca retroalimentación en las preguntas del examen 				
