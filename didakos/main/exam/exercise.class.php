...
class Exercise
{
	...
	function read($id)
	{
		...
		$sql="SELECT a.title,a.description,a.sound,a.type,a.random,a.active, a.results_disabled, b.intentos, b.minimo FROM $TBL_EXERCICES a, $TBL_EXAMS b  WHERE a.id='$id' and a.id=b.id";
	...
	/**
	 * Devuelve el número de intentos permitidos para el examen
	...
	/**
	 * Devuelve el número mínimo de preguntas permitido para el examen
	...
	/**
	 * Cambia el número de intentos del examen
	 *
	...
	/**
	 * Cambia el número mínimo de preguntas del examen
	 *

	 */
	function updateMinimo($minimo)
	{
		$this->minimo=$minimo;
	}
	...
	function createForm ($form)
	{
		...
		// random
		$random = array();
//		$random[] = FormValidator :: createElement ('text', 'randomQuestions',null,"readonly=true",'0');
		$random[] = FormValidator :: createElement ('text', 'randomQuestions',null,null,'0');
		
		$form -> addGroup($random,null,get_lang('RandomQuestions').' : ','<br />');

		// intentos
		$form -> addElement('text', 'exerciseIntentos', get_lang('ExerciseIntentos').' : ','class="input_titles"');

		// mínimo
		$form -> addElement('text', 'exerciseMinimo', get_lang('ExerciseMinimo').' : ','class="input_titles"');
		...
		if($this -> id > 0)
		{
			...
			$defaults['exerciseIntentos'] = $this -> selectIntentos();
			$defaults['exerciseMinimo'] = $this -> selectMinimo();
		}
		else{
			$defaults['exerciseType'] = 1;
//			$defaults['randomQuestions'] = 0;
			$defaults['randomQuestions'] = 10;
			$defaults['exerciseDescription'] = '';
			$defaults['exerciseIntentos'] = 1;
			$defaults['exerciseMinimo'] = 10;
		}
	...
	function processCreation($form)
	{
		...
		$this -> updateIntentos($form -> getSubmitValue('exerciseIntentos'));
		$this -> updateMinimo($form -> getSubmitValue('exerciseMinimo'));
		$this -> save();
