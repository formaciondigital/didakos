...
<div id="logosHeader1"></div><div id="imagenHeader1"></div>
<?php
/*
-----------------------------------------------------------------------------
...
/*if (!empty($_cid) and $_cid != -1 and isset($_course))
{
	...
	?>
	<!-- <a href="<?php// echo api_get_path(WEB_COURSE_PATH).$_course['path']; ?>/index.php" target="_top"> -->
	<?php
...
if (isset ($_cid) and $_cid!=-1 and isset($_course))
{
	...
	switch(api_get_setting('breadcrumbs_course_homepage'))
	{
		...
		case 'course_code':
			...
			break;
		default:
			//Cambio FD, 28/11/2011 - Añadimos el Código del curso al lado del título del curso (para diferenciar varias ediciones de un mismo curso).
			$navigation_item['title'] =  $_course['name']." - [".$_cid."]";
			break;
	}
...
//Insertamos un br para separar la barra de navegación de las pestañas
echo '<br>';
//Fin
...
function get_tabs()
{
	...
	if (api_is_platform_admin(true))
	{
	 ...
	 //07-03-2011 - cambio fd - si el un inspector solo le mostramos la opcion de cursos
        if($_user['status']==DRH)
        {
                $navigation = "";
                $navigation['session_my_space']['url'] = api_get_path(WEB_PATH).'user_portal.php';
	            $navigation['session_my_space']['title'] = get_lang('MyCourses');
        }
