...
function handle_uploaded_multimedia($_course,$uploaded_file,$base_work_dir,$upload_path,$user_id,$to_group_id,$to_user_id,$maxFilledSpace,$unzip=0,$what_if_file_exists='')
 ...
 * this function does the save-work for the documents...
	...
	ES UNA COPIA DE LA ANTERIOR MODIFICADA PARA LOS archivos multimedia	
 ...
 * @return path of the saved file
 */
function handle_uploaded_multimedia($_course,$uploaded_file,$base_work_dir,$upload_path,$user_id,$to_group_id=0,$to_user_id=NULL,$maxFilledSpace='',$unzip=0,$what_if_file_exists='',$output=true)
{
	if(!$user_id) die("Not a valid user.");
				...
				default:
					...
					else
					{
						...
						{							
							...
							//put the document data in the database
							//creamos una funcion nueva add_multimedia, es igual pero con otra tabla de destino
							$document_id = add_multimedia($_course,$file_path,'file',$file_size,$document_name);
							if ($document_id)
							{
								//update document item_property
								api_item_property_update($_course,TOOL_MULTIMEDIA,$document_id,'DocumentAdded',$user_id,$to_group_id,$to_user_id);
							}
							//if the file is in a folder, we need to update all parent folders
							//item_property_update_on_folder($_course,$upload_path,$user_id);
							//display success message to user
...
/**
 * Adds a new multimedia resource to the database   ¡¡NUEVO!!!
 *
 * @param array $_course
 * @param string $path
 * @param string $filetype
 ...
 * @return id if inserted document
 */
function add_multimedia($_course,$path,$filetype,$filesize,$title,$comment=NULL, $readonly=0)
{
	$table_document = Database::get_course_table(TABLE_MULTIMEDIA,$_course['dbName']);
	$sql="INSERT INTO $table_document
	(`target`)
	VALUES ('$path')";
	if(api_sql_query($sql,__FILE__,__LINE__))
