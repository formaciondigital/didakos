...
define('TOOL_QUIZ_EXAM', 'quiz_exam');
...
define('TOOL_COURSE_SERVTECNICO','ServTecnico');
...
function api_protect_admin_script($allow_sessions_admins=false)
{
	if (!api_is_platform_admin($allow_sessions_admins))
	{
        ...
        {
            //OK, is platformAdmin but... it's a gestor?
             if ($_SESSION['is_platformGestor'])
             { 
                include (api_get_path(INCLUDE_PATH)."header.inc.php");
		api_not_allowed();
             }            
...
/**
* Function used to protect a gestor script.
* The function blocks access when the user has no platform gestor rights.
*
* @author Egarcia
*/
function api_protect_gestor_script($allow_sessions_admins=false)
{
	if (!api_is_platform_admin($allow_sessions_admins))
	{
		include (api_get_path(INCLUDE_PATH)."header.inc.php");
...
function api_not_allowed($print_headers = false)
{
	...
	else
	{
		...
		else
		{
			...
			echo '<div align="center">';
			Display :: display_error_message(get_lang('NotAllowed').'<br/><br/><a href="'.$home_url.'" target="_parent">'.get_lang('PleaseLoginAgainFromHomepage').'</a><br/>',false);
			echo '</div>';
			if($print_headers){Display::display_footer();}
			die();
		}
...
{
/*api_mail_html
($recipient_name, $recipient_email, $subject, $message, $sender_name="", $sender_email="", $extra_headers=null)
*/
return api_mail_html("",$to, $subject, $message, "","", $additional_headers);
...
// Funciones añadidas por FD para las redes sociales.
// egarcia 29/07/2011

function api_get_twitter_tokens($user_id)
{	
...
function api_get_facebook_tokens($user_id)
{	
