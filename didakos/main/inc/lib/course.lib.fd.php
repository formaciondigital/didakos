...
Autor: Jesús Zafra
Página incial: course.lib.php (1.8.5)
Página actual: course.lib.fd.php
Modificaciones:

- modificada la funcion  subscribe_user para que añada un registro en la tabla course_rel_user_fd cuando matricula un alumno
...
class CourseManagerFD
{
	...
	*/
	...
	function unsubscribe_user($user_id, $course_code)
	{
		...
		$table_course_user_fd = Database :: get_main_table(TABLE_MAIN_COURSE_USER_FD);
		...
		//egarcia 06/05/10 Incomprensiblemente Dokeos se empeña en borrar tablas que no tienen por que existir... te deja
		//cientos de tablas colgando pero estas las quiere borrar... .
		...
		//CAMBIO FD - eliminamos tambien los registros de la tabla de extension de matriculaciones course_rel_user_fd
		$sql = "DELETE FROM $table_course_user_fd  WHERE user_id IN (".$user_ids.") AND course_code = '".$course_code."'";
		api_sql_query($sql, __FILE__, __LINE__);
		...
		else
		{
			...
			else
			{
				...
				else
				{
				   	  ...
				   	  //return $user_id." * ".$course_code." * ".$status;
	                  //die();
					...
					$add_fecha_matriculacion = "INSERT INTO $course_user_table_fd 
										        SET `course_code` = '$course_code',
										        `user_id`    = '$user_id',
												 f_matriculacion = sysdate()";
					$result2 = api_sql_query($add_fecha_matriculacion, __FILE__, __LINE__);
...
function get_user_list_from_course_code_fd($course_code, $with_session=true, $session_id=0, $limit='', $order_by='')
	{		
		$session_id = intval($session_id);
		$a_users = array();		
		$table_users = Database :: get_main_table(TABLE_MAIN_USER);
		...
		/**CAMBIO FD
	*	devuelve una lista de profesores del curso sin incluir administradores
	*	
	*
	*   
	*
	...
	*	@return array with user info
	*/
	function get_teacher_list_from_course_code_sin_admin($course_code, $with_session=true, $session_id=0, $limit='', $order_by='')
	{		
		$session_id = intval($session_id);
		$a_users = array();		
		$table_users = Database :: get_main_table(TABLE_MAIN_USER);
		...
		if($session_id == 0)
		{
			...
			$sql .= ' LEFT JOIN '.$table_course_user.' as course_rel_user
				        ON user.user_id = course_rel_user.user_id
						AND course_rel_user.course_code="'.Database::escape_string($course_code).'"';
						//CAMBIO FD - añadimos la condicion de que los usuarios sean profesores y no sean admin
			$where[] = ' course_rel_user.course_code IS NOT NULL 
						 and user.status = 1
			             and user.user_id not in(select user_id from '.$table_admin.') ';
		}		
	...
	*/
	...
	function delete_course($code)
	{
		...
		if (!CourseManager :: is_virtual_course_from_system_code($code))
		{
			...
			foreach ($virtual_courses as $index => $virtual_course)
			{
				...
				api_sql_query($sql, __FILE__, __LINE__);
				//CAMBIO FD - eliminamos el registro de la tabla course_rel_user_fd
			    $sql = "DELETE FROM $table_course_user_fd WHERE course_code='".$virtual_course['code']."'";
				api_sql_query($sql, __FILE__, __LINE__);
	...
	function course_exists($course_code)
	{
		...
		/**
	 * conprueba si un curso esta cerrado
	 * @param string course_code
	 * @return true si esta cerrado y si no false
	 */
	function is_course_closed($course_code)
	{
		$sql = 'SELECT visibility FROM '.Database :: get_main_table(TABLE_MAIN_COURSE).' WHERE code="'.Database::escape_string($course_code).'"';
		$rs = api_sql_query($sql,__FILE__,__LINE__);
		
		$resultado=Database::fetch_array($rs);
    ...
    function close_course($curso=0,$accion=2)
	{
	
	   if($accion == 0)//abrir curso
	   {
	   
	     $sql = 'UPDATE '.Database :: get_main_table(TABLE_MAIN_COURSE).' SET  visibility = 1 WHERE code="'.Database::escape_string($curso).'"';
	   }
	   
	    if($accion == 1)//cerrar curso
	   {
	   
	     $sql = 'UPDATE '.Database :: get_main_table(TABLE_MAIN_COURSE).' SET  visibility = 0 WHERE code="'.Database::escape_string($curso).'"';
	   }
	...
	//cierra el acceso (accion=1) o lo abre (accion=0) a un alumno en un curso dado
	//cierra el acceso a todos los alumnos de un curso (accion=2)
	function finalizar_alumno($accion=2,$usuario=0,$curso=0)
	{
	...
	$course_user_table_fd = Database::get_main_table(TABLE_MAIN_COURSE_USER_FD);
	 	...
	 	if($accion == 0)//abrir curso
	   {
	     ...
	     $sql = "UPDATE $course_user_table_fd SET  f_finalizacion = NULL where course_code = '".Database::escape_string($curso)."' and user_id = ".Database::escape_string($usuario);
	   }
	   
	   if($accion == 1)//cerrar curso
	   {
	   
	     $sql = "UPDATE $course_user_table_fd SET  f_finalizacion = sysdate() where course_code = '".Database::escape_string($curso)."' and user_id = ".Database::escape_string($usuario);
	   }
	   if($accion == 2)//cerrar curso completo
	   {
	   
	     $sql = "UPDATE $course_user_table_fd SET  f_finalizacion = sysdate() where course_code = '".Database::escape_string($curso) . "'";
	   }
