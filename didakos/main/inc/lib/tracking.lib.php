...
class Tracking {
	...
	function get_avg_student_score($student_id, $course_code) {
		...
		if(!empty($course['db_name']))
		{
			...
			{
				//SI QUEREMOS QUE EL PORCENTAJE SALGA CON DECIMALES LO PONEMOS UN 2 al ROUND
	...
	function count_login_per_student($student_id, $course_code) { 
	 ...
	 // 26/10/2010 - CAMBIO FD - GRAN CAGADA DE DOKEOS QUE ENVIABA EL PARAMETRO USER_ID Y LO USABA COMO SI FUERA hr_dept_id
	//function get_student_followed_by_drh($hr_dept_id) {
        function get_student_followed_by_drh($user_id) {
		...
		/*$sql = 'SELECT DISTINCT user_id FROM '.$tbl_user.' as user
                ...
                $sql = 'SELECT DISTINCT user_id FROM '.$tbl_user.' as user
			WHERE hr_dept_id IN (
                        SELECT hr_dept_id FROM '.$tbl_user.' WHERE user_id = '.$user_id.')';
		$rs = api_sql_query($sql, __FILE__, __LINE__);
