...
Autor: Jesús Zafra
Página incial: usermanager.lib.php (1.8.5)
Página actual: usermanager.lib.fd.php (1.8.5)
Descripción: Clase para el manejo de usuarios
Modificaciones:
-añadida funcion create_alumno
-añadida funcion create_teletutor
-modificada funcion get_personal_session_course_list. Se recoge la f_finalizacion del alumno.
-añadida la funcion  is_dni_available que comprueba si un dni existe ya en la base de datos
...
class UserManagerFD
{
	...
	function create_user($firstName, $lastName, $status, $email, $loginName, $password, $official_code = '', $language='', $phone = '', $picture_uri = '', $auth_source = PLATFORM_AUTH_SOURCE, $expiration_date = '0000-00-00 00:00:00', $active = 1, $hr_dept_id=0, $extra=null)
	{
		...
		else
		{
		  ...
		  if (UserManagerFD::is_dni_available($official_code)>0)
			return 0;//el dni ya existe
	...
	//procedimiento de creacion de alumnos
	
	
	function create_alumno($firstName, $lastName, $status, $email, $official_code = '', $language='', $phone = '', $auth_source = PLATFORM_AUTH_SOURCE, $expiration_date = '0000-00-00 00:00:00', $active = 1, $hr_dept_id=0, $extra=null)
	{
		global $_user, $userPasswordCrypted;
		...
		//comprobamos que el dni no este ya en la base de datos
		 //return UserManagerFD::is_dni_available($official_code);
		...
		$v_pass = strtoupper(api_generate_password());
		
         //en el campo username introducimos la password ya que debe ser un campo unico
         
		
		$sql = "INSERT INTO $table_user
					                SET lastname = '".Database::escape_string($lastName)."',
					                firstname = '".Database::escape_string($firstName)."',
					                username = '".Database::escape_string($v_pass)."',
					                status = '".Database::escape_string($status)."',
					                password = '".Database::escape_string($v_pass)."',
					                email = '".Database::escape_string($email)."',
					                official_code	= '".Database::escape_string($official_code)."',
					                picture_uri 	=  NULL ,
					                creator_id  	= '".Database::escape_string($creator_id)."',
		...
		//una vez insertado el usuario obtenemos su id de usuario para componer el username
		
		$sql = "select user_id from $table_user where password = '".$v_pass."'";
		$recor = api_sql_query($sql);
		$filas = Database::fetch_array($recor);
		$id_usuario = $filas["user_id"];

		
		//generamos el username con la primera letra del nombre,el apellido y el id de usuario
		
	
		//separamos los apellidos
		$a_apellidos = split(" ", $lastName);
		
		//si la longitud del apellido mayor de 3 caracteres lo utilizamos
		
		for($i=0 ; $i <= sizeof($a_apellidos) ; $i++)
		{
				if(strlen($a_apellidos[$i]) >= 4)
		        {
		          $lastName = trim($a_apellidos[$i]);
				  $hecho = true;
				  break;
		        }
		}
		
		//si ninguna parte del apellido es mayor de 3 caracteres cogemos el primero
		
		if (!$hecho)
		{
		  $lastName = $a_apellidos[0];
		}
		
		//componemos el usuario con la primera letra del apellido,el primer apellido y el id de usuario
				
	    $v_usuario = substr($firstName,0,1).$lastName.$id_usuario;
		
		//limpiamos el usuario de caracteres raros
		
		$caracteres_m = "ÀÁÂÃÄÅàáâãäåÒÓÔÕÖØòóôõöøÈÉÊËèéêëÇçÌÍÎÏìíîïÙÚÛÜùúûüÿ";
        $caracteres_b = "AAAAAAaaaaaaOOOOOOooooooEEEEeeeeCcIIIIiiiiUUUUuuuuy";
        $v_usuario = strtr($v_usuario,$caracteres_m,$caracteres_b);

		
		
		//actualizamos su valor en la base de datos
		
		$sql = "update $table_user set username = '".$v_usuario."' where password = '".$v_pass."'";
		$result2 = api_sql_query($sql);

		
		//si se ha insertado correctamente el usuario y se ha actualizdo correctamente el password,devolvemos el id de usuario
		if ($result && $result2)
		{
			//echo "id returned";
			$return=$id_usuario;
       ...
       // procedimiento para la creacion de teletutores



       function create_teletutor($firstName, $lastName, $status, $email, $official_code = '', $language='', $phone = '',$picture_uri, $auth_source = PLATFORM_AUTH_SOURCE, $expiration_date = '0000-00-00 00:00:00', $active = 1, $hr_dept_id=0, $extra=null)
	   {
		global $_user, $userPasswordCrypted;
		...
		//generamos la password
		$v_pass = api_generate_password();
		
		
         //en nombre de usuario sera 'Teletutor' para todos los formadores
	 ...
	 * Esto es para updatear los datos de los alumnos en la importación masiva de alumnos por csv.
	 * Está capada para que solo se puedan cambiar ciertos datos. 
	 */
	function update_user_csv ($user_id, $firstname, $lastname, $email,  $phone)
	{
	 ...
	 * @return boolean true if the user information was updated
	 */
	function update_user($user_id, $firstname, $lastname, $username = null, $password = null, $auth_source = null, $email, $status, $official_code, $phone, $picture_uri, $expiration_date, $active, $creator_id= null, $hr_dept_id=0, $extra=null)
	{
		global $userPasswordCrypted;
		$table_user = Database :: get_main_table(TABLE_MAIN_USER);
		$sql = "UPDATE $table_user SET
	...
	function is_username_available($username)
	{
		...
		/**
	 * Check if a DNI is available
	 * @param string the wanted DNI
	 * @return boolean true if the wanted DNI is available
	 */
	function is_dni_available($dni)
	{
		$table_user = Database :: get_main_table(TABLE_MAIN_USER);
		$sql = "SELECT official_code FROM $table_user WHERE official_code = '".$dni."'";
		$res = api_sql_query($sql,__FILE__,__LINE__);
	...
	*/
	 ...
	 * @return array All user information as an associative array
	 */
	function get_user_info_by_dni($dni)
	{
		$user_table = Database :: get_main_table(TABLE_MAIN_USER);
		$sql = "SELECT * FROM $user_table WHERE upper(official_code)='".strtoupper(trim($dni))."'";
		$res = api_sql_query($sql,__FILE__,__LINE__);
		if(Database::num_rows($res)>0)
		{
			$user = Database::fetch_array($res);
...
*/
	...
	function get_personal_session_course_list($user_id)
	{
		...
		//añadimos esta
		$tbl_matriculaciones		=Database :: get_main_table(TABLE_MAIN_MATRICULACIONES);
		
		
		
			//## cambio - modificamos la consulta de cursos para que el idioma sea siempre el idioma actual de la plataforma
	
	
				if(isset($_SESSION['_user']['language']))
				{
				//si el alumno ha cambiado el idioma de la plataforma
				$idioma_curso = $_SESSION['_user']['language'];
				
				}else{
				//si no cogemos el 
				$idioma_curso = api_get_setting('platformLanguage');
		...
		$personal_course_list_sql = "SELECT course.code k, course.directory d, course.visual_code c, course.db_name db, course.title i, course.tutor_name t, '".$idioma_curso."' l, course_rel_user.status s, course_rel_user.sort sort, course_rel_user.user_course_cat user_course_cat, m.f_finalizacion f,course.expiration_date expiration, m.f_matriculacion			FROM    ".$tbl_course_user." course_rel_user
										LEFT JOIN ".$tbl_course." course
										ON course.code = course_rel_user.course_code
										LEFT JOIN  " . $tbl_matriculaciones ." m 
										ON m.course_code= course.code and m.user_id='".$user_id."'
		...
		if(api_is_allowed_to_create_course())
		{
			...
			{
				$id_session = $enreg['id'];
				$personal_course_list_sql = "SELECT DISTINCT course.code k, course.directory d, course.visual_code c, course.db_name db, course.title i, CONCAT(user.lastname,' ',user.firstname) t, email, '".$idioma_curso."' l, 1 sort, category_code user_course_cat, date_start, date_end, session.id as id_session, session.name as session_name
