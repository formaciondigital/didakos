...
$admin_table = Database::get_main_table(TABLE_MAIN_ADMIN);
        ...
        if (mysql_num_rows($result) > 0)
        {
			...
			// CAMBIO FD: actualización del idioma del usuario
			$_user ['language'] = $_SESSION ['idioma_acceso'];
			// FIN CAMBIO FD
...
if (isset($cidReset) && $cidReset) // course session data refresh requested or empty data
{
    ...
    if ($cidReq)
    {
        ...
        if (mysql_num_rows($result)>0)
        {
            ...
            // CAMBIO FD: actualización del idioma del curso y del idioma del usuario cuando 
			$_course['language'] = $_SESSION ['idioma_acceso'];
			// FIN CAMBIO FD
