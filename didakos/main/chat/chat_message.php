...
Página incial: chat_message.php (1.8.5)
Página actual: chat_message.php
Descripción:  Añadimos a cada line la fecha y la hora
	...
	*/
	...
	if($sent)
	{
		...
		if(!empty($message))
		{
			...
			//##cambio	añadimos fecha y hora
			if($isMaster)
			{
				fputs($fp,'<span id="chat_login_name"><b>'.$firstname.' '.$lastname.'</b> ('. $fechaHora.')</span> : '.$message.'<br>'."\n");	 		
				
			}
			else
			{
				fputs($fp,'<b>'.$firstname.' '.$lastname.'</b> ('.$fechaHora.')</span> : '.$message.'<br>'."\n");	
			}
			//##cambio	añadimos fecha y hora
