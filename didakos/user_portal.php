...
Página incial: user_portal.php (1.8.5)
Página actual: user_portal.php
Descripción: Eliminamos el menu lateral. buscar ##cambio para encontrarlo
Añadimos herramienta dmail para nucleo v02 29/03/10
...
include_once (api_get_path(LIBRARY_PATH).'usermanager.lib.fd.php');
...
$main_matriculaciones_table 	= Database :: get_main_table(TABLE_MAIN_MATRICULACIONES);
...
**********************************************************
	Egarcia 25/11/10
	Antes de continuar (pintar cabeceras) añadimos una nueva funcionalidad.
	Si el usuario entra en la lista de cursos y solamente tiene un curso 
	redireccionaremos a la página principal del mismo.
	15/07/11
	Modifico para que tenga en cuenta si el curso está o no cerrado.
**********************************************************
*/

$personal_course_list = UserManagerFD::get_personal_session_course_list($_user['user_id']);

if ( count($personal_course_list)==1)
{
	$course_code = $personal_course_list[0][0];
	$course_access_settings = CourseManager :: get_access_settings($course_code);
	// Obtenemos todas las fechas y la visibilidad
	$visibility = $course_access_settings['visibility'];
	$fecha_finalizacion = $personal_course_list[0]['f'];//fecha de finalizacion a nivel de alumno (en la tabla course_rel_user_fd)
	$fecha_matriculacion = $personal_course_list[0]['f_matriculacion'];//fecha de matriculacion a nivel de alumno (en la tabla course_rel_user_fd)

	// Función que devuelve el estado del curso.
	if (getCourseStatus ($visibility, $fecha_matriculacion, $fecha_finalizacion)=="true")
	{
...
//27-10-2010 - cambio fd - si el usuario es responsable de recursos humanos redirijimos a la parte de los informes
/*if($_user['status']==DRH){
header('Location:'.api_get_path(WEB_PATH).'main/mySpace/indexDRH.php');    
}*/
...
/**
* Función que devuelve el estado de un curso, si está abierto devuelve true, en caso contrario devuelve mensaje
*/

function getCourseStatus ($visibility, $fecha_matriculacion, $fecha_finalizacion)
{
	if($visibility==0)
	{ 
		// Curso cerrado
	...
	{
		//si el curso no esta cerrado
		//comprobamos si ya ha llegado la fecha de inicio de la matriculacion,si no ha pasado no le permitimos el acceso
		if ( ($fecha_matriculacion!='') && (intval(strtotime("now")) -  intval(strtotime($fecha_matriculacion)) <=0 ) )
			{
				// El curso aún no está abierto
				// quedan  (intval(strtotime("now")) -  intval(strtotime($fecha_matriculacion)) Segundos para el inicio
	                  	return "  [Pr&oacute;ximo Comienzo]";
			}
			else
			{
				//el curso ha comenzado ya y no esta cerrado
				//comprobamos que no haya pasado ya la fecha de fin del la matriculacion

		      		if ($fecha_finalizacion!='' && (strtotime($fecha_finalizacion) <= strtotime("now")))
				{
...
/*
Una funcion para calcular los dias entre dos fechas dadas
se les pasa en formato yyyy/mm/dd
*/
...
function get_logged_user_course_html($my_course)
{
	...
	//##cambio insertamos la fecha de finalizacion
	$course_fecha_finalizacion = $my_course['f'];//fecha de finalizacion a nivel de alumno (en la tabla course_rel_user_fd)

	$course_fecha_matriculacion = $my_course['f_matriculacion'];//fecha de matriculacion a nivel de alumno (en la tabla course_rel_user_fd)
	$fecha_actual = getdate();
	$fecha_fin_curso = $my_course['expiration'];//fecha de finalizacion global del curso (en la tabla course)
	//##
	...
	//##cambio - comprobamos que el alumno tenga acceso a ese curso
	$acceso = getCourseStatus ($course_visibility, $course_fecha_matriculacion, $course_fecha_finalizacion);

	/*
	//echo "curso:".$fecha_fin_curso."matriculacion:".$course_fecha_matriculacion."finalizacion:".$course_fecha_finalizacion;
	//die();
		//comprobamos que el curso no este cerrado en la tabla course	
		if(strtotime($fecha_fin_curso) < strtotime(time()))
		{ 
		  $acceso = false;
		  $mensaje_cierre = "  [Curso Cerrado]";
		}else{//si el curso no esta cerrado
	
			  	//comprobamos si ya ha llegado la fecha de inicio de la matriculacion,si no ha pasado no le permitimos el acceso
					if ($course_fecha_matriculacion!='' && (strtotime("now") < strtotime($course_fecha_matriculacion)))
					{
					   	  $acceso = false;
			          $mensaje_cierre = "  [Pr&oacute;ximo Comienzo]";
					}else{//el curso ha comenzado ya y no esta cerrado
				
								//comprobamos que no haya pasado ya la fecha de fin del la matriculacion
			      				if ($course_fecha_finalizacion!='' && (strtotime($course_fecha_finalizacion) < strtotime("now")))
								{
									  $acceso = false;
									  $mensaje_cierre = "  [Curso Finalizado]";
								}
	...
	*/	
	...
	if ($course_visibility != COURSE_VISIBILITY_CLOSED || $user_in_course_status == COURSEMANAGER)
	{
		...
		else
		{
					...
					//##cambio
			//Aqui vemos si el curso esta finalizado y no se le debe dar acceso
				if($acceso == "true")
				{
				...
				{
				     $result .= $course_display_title.$acceso;
		         }
		}
	}
	else
	...
	//##cambio - si el curso esta cerrado no se le muestran las novedades
	if($acceso=="true")
	{
...
function show_notification($my_course)
{
	...
	// Para las notificaciones de #dmail egarcia
	$t_dmail_main = Database::get_course_table(TABLE_DMAIL,$course_database);
	...
	if (isset ($notifications))
	{
		while (list ($key, $notification) = each($notifications))
		{
			...
			$retvalue .= '<a href="'.api_get_path(WEB_CODE_PATH).$notification['link'].'?cidReq='.$my_course['k'].'&amp;ref='.$notification['ref'].'">'.'<img title="-- '.get_lang(ucfirst($notification['tool'])).' -- '.get_lang('_title_notification').": ".get_lang($type)." ($lastDate).\"".' src="'.api_get_path(WEB_CODE_PATH).'img/'. str_replace("png","gif",$notification['image']).'" border="0" align="absbottom" /></a>&nbsp;';
		}
	}

	// #dmail buscamos dmail sin leer
	$sql = "select count(*) as total from $t_dmail_main where recibe=$user_id and leido=false and borrado=false and id_carpeta=1";
	$res = api_sql_query($sql, __FILE__, __LINE__);
	$obj = Database::fetch_object($res);
	if ( $obj->total > 0 )
	{
	$retvalue .= '<a href="'.api_get_path(WEB_CODE_PATH).'dmail/index.php?cidReq='.$my_course['k'].'&dir=recibidos"><img title="Tiene Dmail sin leer ('.$obj->total.')" src="'.api_get_path(WEB_CODE_PATH).'img/dmail.gif" border="0" align="absbottom" /></a>&nbsp;';
	}
...
else
{
	...
	foreach ($personal_course_list as $my_course)
	{
		...
		$thisCourseFechaFinalizacion = $my_course['f'];
